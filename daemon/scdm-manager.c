...
#define SCDM_MANAGER_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), SCDM_TYPE_MANAGER, ScdmManagerPrivate))

#define SCDM_DBUS_PATH             "/io/github/scarecrow_de/DisplayManager"
#define SCDM_MANAGER_PATH          SCDM_DBUS_PATH "/Manager"
#define SCDM_MANAGER_DISPLAYS_PATH SCDM_DBUS_PATH "/Displays"
...
struct ScdmManagerPrivate
{
        ScdmDisplayStore        *display_store;
        ScdmLocalDisplayFactory *local_factory;
#ifdef HAVE_LIBXDMCP
        ScdmXdmcpDisplayFactory *xdmcp_factory;
#endif
        ScdmDisplay             *automatic_login_display;
        GList                  *user_sessions;
...
static void     scdm_manager_class_init  (ScdmManagerClass *klass);
static void     scdm_manager_init        (ScdmManager      *manager);
static void     scdm_manager_dispose     (GObject         *object);

static ScdmSession *create_user_session_for_display (ScdmManager *manager,
                                                    ScdmDisplay *display,
                                                    uid_t       allowed_user);
static void     start_user_session (ScdmManager                *manager,
                                    StartUserSessionOperation *operation);
static void     clean_user_session (ScdmSession *session);

static gpointer manager_object = NULL;

static void manager_interface_init (ScdmDBusManagerIface *interface);

G_DEFINE_TYPE_WITH_CODE (ScdmManager,
                         scdm_manager,
                         SCDM_DBUS_TYPE_MANAGER_SKELETON,
                         G_IMPLEMENT_INTERFACE (SCDM_DBUS_TYPE_MANAGER,
                                                manager_interface_init)
                         G_ADD_PRIVATE (ScdmManager));
...
static gboolean
is_login_session (ScdmManager  *self,
...
static gboolean
session_unlock (ScdmManager *manager,
                const char *ssid)
{
        GError *error = NULL;
        GVariant *reply;
        ...
        reply = g_dbus_connection_call_sync (manager->priv->connection,
                                             ...
                                             &error);
        if (reply == NULL) {
                g_debug ("ScdmManager: logind 'UnlockSession' %s raised:\n %s\n\n",
...
static ScdmSession *
find_session_for_user_on_seat (ScdmManager *manager,
                               const char *username,
                               const char *seat_id,
                               ScdmSession *dont_count_session)
{
        GList *node;

        for (node = manager->priv->user_sessions; node != NULL; node = node->next) {
                ScdmSession *candidate_session = node->data;
                const char *candidate_username, *candidate_seat_id, *candidate_session_id;

                candidate_session_id = scdm_session_get_session_id (candidate_session);

                if (candidate_session == dont_count_session) {
                        g_debug ("ScdmSession: Ignoring session %s as requested",
                                 candidate_session_id);
                        continue;
                }

                if (!scdm_session_is_running (candidate_session)) {
                        g_debug ("ScdmSession: Ignoring session %s as it isn't running",
                                 candidate_session_id);
                        continue;
                }

                candidate_username = scdm_session_get_username (candidate_session);
                candidate_seat_id = scdm_session_get_display_seat_id (candidate_session);

                g_debug ("ScdmManager: Considering session %s on seat %s belonging to user %s",
                         candidate_session_id,
                         candidate_seat_id,
                         candidate_username);
                ...
                if (g_strcmp0 (candidate_username, username) == 0 &&
                    g_strcmp0 (candidate_seat_id, seat_id) == 0) {
                        g_debug ("ScdmManager: yes, found session %s", candidate_session_id);
                        return candidate_session;
                }

                g_debug ("ScdmManager: no, will not use session %s", candidate_session_id);
        }

        g_debug ("ScdmManager: no matching sessions found");
        return NULL;
}

static gboolean
is_remote_session (ScdmManager  *self,
                   const char  *session_id,
                   GError     **error)
{
        char *seat;
        ...
        if (ret < 0 && ret != -ENXIO) {
                g_debug ("ScdmManager: Error while retrieving seat for session %s: %s",
...
static void
get_display_and_details_for_bus_sender (ScdmManager       *self,
                                        GDBusConnection  *connection,
                                        const char       *sender,
                                        ScdmDisplay      **out_display,
                                        char            **out_seat_id,
                                        char            **out_session_id,
                                        char            **out_tty,
                                        GPid             *out_pid,
                                        ...
                                        gboolean         *out_is_remote)
{
        ...
        ret = scdm_dbus_get_pid_for_name (sender, &pid, &error);

        if (!ret) {
                g_debug ("ScdmManager: Error while retrieving pid for sender: %s",
                         error->message);
                g_error_free (error);
                goto out;
        }
        ...
        ret = scdm_dbus_get_uid_for_name (sender, &caller_uid, &error);

        if (!ret) {
                g_debug ("ScdmManager: Error while retrieving uid for sender: %s",
                         error->message);
                g_error_free (error);
                goto out;
        }

        ret = scdm_find_display_session (pid, caller_uid, &session_id, &error);

        if (!ret) {
                g_debug ("ScdmManager: Unable to find display session for uid %d: %s",
                         (int) caller_uid,
                         error->message);
                g_error_free (error);
                goto out;
        ...
        if (out_is_login_screen != NULL) {
                *out_is_login_screen = is_login_session (self, session_id, &error);
                ...
                if (error != NULL) {
                        g_debug ("ScdmManager: Error while checking if sender is login screen: %s",
                                 error->message);
                        g_error_free (error);
                        goto out;
                }
        ...
        if (!get_uid_for_session_id (session_id, &session_uid, &error)) {
                g_debug ("ScdmManager: Error while retrieving uid for session: %s",
                         error->message);
                g_error_free (error);
                goto out;
        }
        ...
        if (caller_uid != session_uid) {
                g_debug ("ScdmManager: uid for sender and uid for session don't match");
                goto out;
        }
        ...
        if (out_seat_id != NULL) {
                *out_seat_id = get_seat_id_for_session_id (session_id, &error);
                ...
                if (error != NULL) {
                        g_debug ("ScdmManager: Error while retrieving seat id for session: %s",
                                 error->message);
                        g_clear_error (&error);
                }
        }
        ...
        if (out_is_remote != NULL) {
                *out_is_remote = is_remote_session (self, session_id, &error);
                ...
                if (error != NULL) {
                        g_debug ("ScdmManager: Error while retrieving remoteness for session: %s",
                                 error->message);
                        g_clear_error (&error);
                }
        }
        ...
        if (out_tty != NULL) {
                *out_tty = get_tty_for_session_id (session_id, &error);
                ...
                if (error != NULL) {
                        g_debug ("ScdmManager: Error while retrieving tty for session: %s",
                                 error->message);
                        g_clear_error (&error);
                }
        }

        display = scdm_display_store_find (self->priv->display_store,
...
static gboolean
switch_to_compatible_user_session (ScdmManager *manager,
                                   ScdmSession *session,
                                   gboolean    fail_if_already_switched)
{
        ...
        ScdmSession *existing_session;

        ret = FALSE;

        username = scdm_session_get_username (session);
        seat_id = scdm_session_get_display_seat_id (session);
        ...
        if (existing_session != NULL) {
                ssid_to_activate = scdm_session_get_session_id (existing_session);
                if (seat_id != NULL) {
                        res = scdm_activate_session_by_id (manager->priv->connection, seat_id, ssid_to_activate);
                        if (! res) {
                                g_debug ("ScdmManager: unable to activate session: %s", ssid_to_activate);
                                goto out;
                        }
                }
                ...
                if (!res) {
                        /* this isn't fatal */
                        g_debug ("ScdmManager: unable to unlock session: %s", ssid_to_activate);
                }
        } else {
                goto out;
        }
...
static ScdmDisplay *
get_display_for_user_session (ScdmSession *session)
{
        return g_object_get_data (G_OBJECT (session), "scdm-display");
}

static ScdmSession *
get_user_session_for_display (ScdmDisplay *display)
{
        if (display == NULL) {
                return NULL;
        }

        return g_object_get_data (G_OBJECT (display), "scdm-user-session");
}

static gboolean
add_session_record (ScdmManager    *manager,
                    ScdmSession    *session,
                    GPid           pid,
                    SessionRecord  record)
{
        ...
        switch (record) {
            ...
            case SESSION_RECORD_LOGOUT:
                scdm_session_record_logout (pid,
                                           username,
                                           hostname,
                                           display_name,
                                           display_device);
            ...
            case SESSION_RECORD_FAILED:
                scdm_session_record_failed (pid,
                                           username,
                                           hostname,
                                           display_name,
                                           display_device);
...
static ScdmSession *
find_user_session_for_display (ScdmManager *self,
                               ScdmDisplay *display)
{
        ...
        while (node != NULL) {
                ScdmSession *session = node->data;
                ScdmDisplay *candidate_display;
...
static gboolean
scdm_manager_handle_register_display (ScdmDBusManager        *manager,
                                     GDBusMethodInvocation *invocation,
                                     GVariant              *details)
{
        ScdmManager      *self = SCDM_MANAGER (manager);
        const char      *sender;
        GDBusConnection *connection;
        ScdmDisplay      *display = NULL;
        ScdmSession      *session;
        ...
        g_debug ("ScdmManager: trying to register new display");
        ...
        if (session != NULL) {
                ...
                /* FIXME: this should happen in scdm-session.c when the session is opened
                 */
                if (tty != NULL)
                        g_object_set (G_OBJECT (session), "display-device", tty, NULL);

                pid = scdm_session_get_pid (session);
...
static gboolean
scdm_manager_handle_register_session (ScdmDBusManager        *manager,
                                     GDBusMethodInvocation *invocation,
                                     GVariant              *details)
{
        ScdmManager      *self = SCDM_MANAGER (manager);
        ...
        g_debug ("ScdmManager: trying to register new session on display %p", display);

        if (display != NULL)
                g_object_set (G_OBJECT (display), "session-registered", TRUE, NULL);
        else
                g_debug ("ScdmManager: No display, not registering");

        scdm_dbus_manager_complete_register_session (SCDM_DBUS_MANAGER (manager),
                                                    invocation);
...
static gboolean
scdm_manager_handle_open_session (ScdmDBusManager        *manager,
                                 GDBusMethodInvocation *invocation)
{
        ScdmManager       *self = SCDM_MANAGER (manager);
        const char       *sender;
        GDBusConnection  *connection;
        ScdmDisplay       *display = NULL;
        ScdmSession       *session = NULL;
        const char       *address;
        GPid              pid = 0;
        uid_t             uid = (uid_t) -1;
        uid_t             allowed_user;

        g_debug ("ScdmManager: trying to open new session");
        ...
        if (session == NULL) {
                ...
                g_debug ("ScdmSession: Considering session %s for username %s",
                         scdm_session_get_session_id (session),
                         scdm_session_get_username (session));

                if (scdm_session_is_running (session)) {
                        g_debug ("ScdmSession: the session is running, and therefore can't be used");
                        g_dbus_method_invocation_return_error_literal (invocation,
                                                                       G_DBUS_ERROR,
                                                                       G_DBUS_ERROR_ACCESS_DENIED,
        ...
        allowed_user = scdm_session_get_allowed_user (session);
        ...
        address = scdm_session_get_server_address (session);
        ...
        scdm_dbus_manager_complete_open_session (SCDM_DBUS_MANAGER (manager),
                                                invocation,
                                                address);
        return TRUE;
}
...
static void
close_transient_session (ScdmManager *self,
                         ScdmSession *session)
{
        GPid pid;
        pid = GPOINTER_TO_UINT (g_object_get_data (G_OBJECT (session), "caller-pid"));
        scdm_session_close (session);
...
on_reauthentication_client_connected (ScdmSession              *session,
                                      ...
                                      ScdmManager              *self)
{
        g_debug ("ScdmManager: client connected to reauthentication server");
}
...
on_reauthentication_client_disconnected (ScdmSession              *session,
                                         ...
                                         ScdmManager              *self)
{
        g_debug ("ScdmManger: client disconnected from reauthentication server");
        close_transient_session (self, session);
}

static void
on_reauthentication_client_rejected (ScdmSession              *session,
                                     GCredentials            *credentials,
                                     GPid                     pid_of_client,
                                     ScdmManager              *self)
{
        ...
        g_debug ("ScdmManger: client with pid %ld rejected from reauthentication server", (long) pid_of_client);

        if (scdm_session_client_is_connected (session)) {
...
on_reauthentication_cancelled (ScdmSession *session,
                               ScdmManager *self)
{
        g_debug ("ScdmManager: client cancelled reauthentication request");
        close_transient_session (self, session);
}
...
on_reauthentication_conversation_started (ScdmSession *session,
                                          ...
                                          ScdmManager *self)
{
        g_debug ("ScdmManager: reauthentication service '%s' started",
                 service_name);
}
...
on_reauthentication_conversation_stopped (ScdmSession *session,
                                          ...
                                          ScdmManager *self)
{
        g_debug ("ScdmManager: reauthentication service '%s' stopped",
                 service_name);
}
                                           ...
                                           ScdmManager *self)
{
        const char *session_id;
        session_id = g_object_get_data (G_OBJECT (session), "caller-session-id");
        g_debug ("ScdmManager: reauthenticated user in unmanaged session '%s' with service '%s'",
...
static char *
open_temporary_reauthentication_channel (ScdmManager            *self,
                                         char                  *seat_id,
                                         char                  *session_id,
                                         GPid                   pid,
                                         uid_t                  uid,
                                         gboolean               is_remote)
{
        ...
        g_debug ("ScdmSession: Created session for temporary reauthentication channel for user %d (seat %s)",
                 (int) uid,
                 seat_id);
...
static gboolean
scdm_manager_handle_open_reauthentication_channel (ScdmDBusManager        *manager,
                                                  GDBusMethodInvocation *invocation,
                                                  const char            *username)
{
        ScdmManager       *self = SCDM_MANAGER (manager);
        ...
        g_debug ("ScdmManager: trying to open reauthentication channel for user %s", username);
        ...
        if (is_login_screen) {
                g_debug ("ScdmManager: looking for login screen session for user %s on seat %s", username, seat_id);
                session = find_session_for_user_on_seat (self,
                                                         username,
                                                         seat_id,
                                                         NULL);
        } else {
                g_debug ("ScdmManager: looking for user session on display");
                session = get_user_session_for_display (display);
        }

        if (session != NULL && scdm_session_is_running (session)) {
                scdm_session_start_reauthentication (session, pid, uid);
                ...
                address = open_temporary_reauthentication_channel (self,
                                                                   ...
                                                                   uid,
                                                                   is_remote);
                scdm_dbus_manager_complete_open_reauthentication_channel (SCDM_DBUS_MANAGER (manager),
                                                                         invocation,
                                                                         address);
                g_free (address);
        }
...
static void
manager_interface_init (ScdmDBusManagerIface *interface)
{
        interface->handle_register_display = scdm_manager_handle_register_display;
        interface->handle_register_session = scdm_manager_handle_register_session;
        interface->handle_open_session = scdm_manager_handle_open_session;
        interface->handle_open_reauthentication_channel = scdm_manager_handle_open_reauthentication_channel;
}

static gboolean
display_is_on_seat0 (ScdmDisplay *display)
{
...
static gboolean
get_timed_login_details (ScdmManager *manager,
                         char      **usernamep,
                         int        *delayp)
{
        gboolean res;
        ...
        res = scdm_settings_direct_get_boolean (SCDM_KEY_TIMED_LOGIN_ENABLE, &enabled);
        if (res && ! enabled) {
                goto out;
        }

        res = scdm_settings_direct_get_string (SCDM_KEY_TIMED_LOGIN_USER, &username);
        ...
        delay = 0;
        res = scdm_settings_direct_get_int (SCDM_KEY_TIMED_LOGIN_DELAY, &delay);
 ...
 out:
        if (enabled) {
                g_debug ("ScdmDisplay: Got timed login details for display: %d %s %d",
                         enabled,
                         username,
                         delay);
        } else {
                g_debug ("ScdmDisplay: Got timed login details for display: 0");
        }
...
static gboolean
get_automatic_login_details (ScdmManager *manager,
                             char      **usernamep)
{
        gboolean res;
        gboolean enabled;
        ...
        res = scdm_settings_direct_get_boolean (SCDM_KEY_AUTO_LOGIN_ENABLE, &enabled);
        if (res && enabled) {
            res = scdm_settings_direct_get_string (SCDM_KEY_AUTO_LOGIN_USER, &username);
        }
 ...
 out:
        if (enabled) {
                g_debug ("ScdmDisplay: Got automatic login details for display: %d %s",
                         enabled,
                         username);
        } else {
                g_debug ("ScdmDisplay: Got automatic login details for display: 0");
        }
...
static const char *
get_username_for_greeter_display (ScdmManager *manager,
                                  ScdmDisplay *display)
{
        ...
        if (doing_initial_setup) {
                ...
                return SCDM_USERNAME;
        }
}

static void
set_up_automatic_login_session (ScdmManager *manager,
                                ScdmDisplay *display)
{
        ScdmSession *session;
        ...
        g_debug ("ScdmManager: Starting automatic login conversation");
        scdm_session_start_conversation (session, "scdm-autologin");
}

static void
set_up_chooser_session (ScdmManager *manager,
                        ScdmDisplay *display)
{
        ...
        if (!scdm_get_pwent_for_name (allowed_user, &passwd_entry)) {
                g_warning ("ScdmManager: couldn't look up username %s",
                           allowed_user);
        ...
        scdm_display_start_greeter_session (display);
}

static void
set_up_greeter_session (ScdmManager *manager,
                        ScdmDisplay *display)
{
        const char *allowed_user;
        struct passwd *passwd_entry;
...
static void
set_up_automatic_login_session_if_user_exists (ScdmManager *manager,
                                               ScdmDisplay *display,
                                               ActUser    *user)
{
...
static void
set_up_session (ScdmManager *manager,
                ScdmDisplay *display)
{
...
static void
start_user_session (ScdmManager *manager,
                    StartUserSessionOperation *operation)
{
        ScdmDisplay *display;
        ...
        if (display != NULL) {
                ...
                if (is_connected) {
                        auth_file = NULL;
                        username = scdm_session_get_username (operation->session);
                        scdm_display_add_user_authorization (display,
                                                            username,
                                                            &auth_file,
                                                            NULL);
        ...
        scdm_session_start_session (operation->session,
...
static void
create_display_for_user_session (ScdmManager *self,
                                 ScdmSession *session,
                                 const char *session_id)
{
        ScdmDisplay *display;
        /* at the moment we only create ScdmLocalDisplay objects on seat0 */
        ...
        g_object_set (G_OBJECT (display),
                      ...
                      NULL);
        scdm_display_store_add (self->priv->display_store,
                               display);
        g_object_set_data (G_OBJECT (session), "scdm-display", display);
...
chown_initial_setup_home_dir (void)
{
        ...
        if (!scdm_get_pwent_for_name (INITIAL_SETUP_USERNAME, &pwe)) {
...
{
        ScdmManager *self = operation->manager;
        gboolean migrated;
        ...
        g_debug ("ScdmManager: start or jump to session");
        ...
        g_debug ("ScdmManager: migrated: %d", migrated);
        if (migrated) {
                /* We don't stop the manager here because
                   when Xorg exits it switches to the VT it was
                   started from.  That interferes with fast
                   user switching. */
                scdm_session_reset (operation->session);
                destroy_start_user_session_operation (operation);
                goto out;
        }
        ...
        if (scdm_session_get_display_mode (operation->session) == GDM_SESSION_DISPLAY_MODE_REUSE_VT) {
                ...
                if (doing_initial_setup) {
                        ...
                        if (!g_file_set_contents (ALREADY_RAN_INITIAL_SETUP_ON_THIS_BOOT,
                                                  ...
                                                  1,
                                                  &error)) {
                                g_warning ("ScdmDisplay: Could not write initial-setup-done marker to %s: %s",
                                           ALREADY_RAN_INITIAL_SETUP_ON_THIS_BOOT,
                                           error->message);
                                g_clear_error (&error);
                        }
                } else {
                        g_debug ("ScdmManager: session has its display server, reusing our server for another login screen");
                }

                /* The user session is going to follow the session worker
                 * into the new display. Untie it from this display and
                 * create a new session for a future user login. */
                allowed_uid = scdm_session_get_allowed_user (operation->session);
                g_object_set_data (G_OBJECT (display), "scdm-user-session", NULL);
                g_object_set_data (G_OBJECT (operation->session), "scdm-display", NULL);
                ...
                if (g_strcmp0 (operation->service_name, "scdm-autologin") == 0 &&
	            !scdm_session_client_is_connected (operation->session)) {
                        /* remove the unused prepared greeter display since we're not going
                         * to have a greeter */
                        scdm_display_store_remove (self->priv->display_store, display);
                        g_object_unref (display);

                        self->priv->automatic_login_display = g_object_get_data (G_OBJECT (operation->session), "scdm-display");
...
static void
queue_start_user_session (ScdmManager *manager,
                          ScdmSession *session,
                          const char *service_name)
{
        StartUserSessionOperation *operation;
...
static void
start_user_session_if_ready (ScdmManager *manager,
                             ScdmSession *session,
                             const char *service_name)
{
...
static void
on_session_authentication_failed (ScdmSession *session,
                                  const char *service_name,
...
static void
on_user_session_opened (ScdmSession       *session,
                        const char       *service_name,
                        const char       *session_id,
                        ScdmManager       *manager)
{
        manager->priv->user_sessions = g_list_append (manager->priv->user_sessions,
                                                      g_object_ref (session));
        if (g_strcmp0 (service_name, "scdm-autologin") == 0 &&
            !scdm_session_client_is_connected (session)) {
...
static void
on_user_session_started (ScdmSession      *session,
                         const char      *service_name,
                         GPid             pid,
                         ScdmManager      *manager)
{
        g_debug ("ScdmManager: session started %d", pid);
...
static void
remove_user_session (ScdmManager *manager,
                     ScdmSession *session)
{
        GList *node;
        ScdmDisplay *display;
...
static void
on_session_start_failed (ScdmSession *session,
                         const char *service_name,
                         const char *message,
                         ScdmManager *manager)
{
        g_debug ("ScdmManager: session failed to start: %s", message);
        remove_user_session (manager, session);
}

static void
on_user_session_exited (ScdmSession *session,
                        int         code,
                        ScdmManager *manager)
{
        GPid pid;

        g_debug ("ScdmManager: session exited with status %d", code);
        pid = scdm_session_get_pid (session);
...
static void
on_user_session_died (ScdmSession *session,
                      int         signal_number,
                      ScdmManager *manager)
{
        g_debug ("ScdmManager: session died with signal %s", strsignal (signal_number));
        remove_user_session (manager, session);
}

static char *
get_display_device (ScdmManager *manager,
                    ScdmDisplay *display)
{
...
static void
on_session_reauthenticated (ScdmSession *session,
                            const char *service_name,
                            ScdmManager *manager)
{
        ...
        if (scdm_session_get_display_mode (session) == GDM_SESSION_DISPLAY_MODE_REUSE_VT) {
                ...
                seat_id = scdm_session_get_display_seat_id (session);
                if (scdm_get_login_window_session_id (seat_id, &session_id)) {
                        ScdmDisplay *display = scdm_display_store_find (manager->priv->display_store,
                                                                      lookup_by_session_id,
                                                                      (gpointer) session_id);
...
static void
on_session_client_ready_for_session_to_start (ScdmSession      *session,
                                              const char      *service_name,
                                              ...
                                              ScdmManager      *manager)
{
        ...
        if (client_is_ready) {
                g_debug ("ScdmManager: Will start session when ready");
        } else {
                g_debug ("ScdmManager: Will start session when ready and told");
        }
...
static void
on_session_client_connected (ScdmSession      *session,
                             GCredentials    *credentials,
                             GPid             pid_of_client,
                             ScdmManager      *manager)
{
        ...
        g_debug ("ScdmManager: client with pid %d connected", (int) pid_of_client);

        if (scdm_session_is_running (session)) {
                const char *session_username;
                session_username = scdm_session_get_username (session);
                g_debug ("ScdmManager: ignoring connection, since session already running (for user %s)",
                         session_username);
                return;
        }
        ...
        scdm_session_set_timed_login_details (session, username, delay);

        g_debug ("ScdmManager: Starting automatic login conversation (for timed login)");
        scdm_session_start_conversation (session, "scdm-autologin");
...
static void
on_session_client_disconnected (ScdmSession   *session,
                                GCredentials *credentials,
                                GPid          pid_of_client,
                                ScdmManager   *manager)
{
        g_debug ("ScdmManager: client with pid %d disconnected", (int) pid_of_client);
}
...
static void
queue_session_reset (ScdmManager *manager,
                     ScdmSession *session)
{
...
static void
on_session_cancelled (ScdmSession  *session,
                      ScdmManager  *manager)
{
        g_debug ("ScdmManager: Session was cancelled");
        queue_session_reset (manager, session);
}

static void
on_session_conversation_started (ScdmSession *session,
                                 const char *service_name,
                                 ScdmManager *manager)
{
        ScdmDisplay *display;
        ...
        g_debug ("ScdmManager: session conversation started for service %s on session", service_name);

        if (g_strcmp0 (service_name, "scdm-autologin") != 0) {
                g_debug ("ScdmManager: ignoring session conversation since its not automatic login conversation");
                return;
        }
        ...
        if (display == NULL) {
                g_debug ("ScdmManager: conversation has no associated display");
                return;
        }
        ...
        g_debug ("ScdmManager: begin auto login for user '%s'", username);

        /* service_name will be "scdm-autologin"
         */
        scdm_session_setup_for_user (session, service_name, username);

        g_free (username);
}

static void
on_session_conversation_stopped (ScdmSession *session,
                                 const char *service_name,
                                 ScdmManager *manager)
{
        g_debug ("ScdmManager: session conversation '%s' stopped", service_name);
}

static void
on_session_reauthentication_started (ScdmSession *session,
                                     int         pid_of_caller,
                                     const char *address,
                                     ScdmManager *manager)
{
        ...
        g_debug ("ScdmManager: reauthentication started");
...
static void
clean_user_session (ScdmSession *session)
{
        g_object_set_data (G_OBJECT (session), "scdm-display", NULL);
        g_object_unref (session);
}

static ScdmSession *
create_user_session_for_display (ScdmManager *manager,
                                 ScdmDisplay *display,
                                 uid_t       allowed_user)
{
        ScdmSession *session;
        ...
        g_debug ("ScdmSession: Created user session for user %d on display %s (seat %s)",
                 (int) allowed_user,
                 display_id,
                 display_seat_id);
                  ...
                  ScdmManager      *manager)
{
        ...
        if (display != NULL) {
                g_dbus_object_manager_server_export (manager->priv->object_manager,
                                                     scdm_display_get_object_skeleton (display));
...
GQuark
scdm_manager_error_quark (void)
{
        static GQuark ret = 0;
        if (ret == 0) {
                ret = g_quark_from_static_string ("scdm_manager_error");
        }
...
/*
  ...
  /io/github/scarecrow_de/DisplayManager/Displays \
  org.freedesktop.ObjectManager.GetAll
*/
gboolean
scdm_manager_get_displays (ScdmManager *manager,
                          GPtrArray **displays,
                          GError    **error)
{
        ...
        *displays = g_ptr_array_new ();
        scdm_display_store_foreach (manager->priv->display_store,
                                   (ScdmDisplayStoreFunc)listify_display_ids,
                                   displays);
...
void
scdm_manager_stop (ScdmManager *manager)
{
...
void
scdm_manager_start (ScdmManager *manager)
{
        g_debug ("ScdmManager: GDM starting to manage displays");
...
static gboolean
register_manager (ScdmManager *manager)
{
        GError *error = NULL;
        ...
        object_server = g_dbus_object_manager_server_new (SCDM_MANAGER_DISPLAYS_PATH);
...
static void
scdm_manager_set_property (GObject      *object,
                          guint         prop_id,
                          const GValue  *value,
                          GParamSpec    *pspec)
{
        ...
        self = SCDM_MANAGER (object);

        switch (prop_id) {
        case PROP_XDMCP_ENABLED:
                scdm_manager_set_xdmcp_enabled (self, g_value_get_boolean (value));
                break;
        case PROP_SHOW_LOCAL_GREETER:
                scdm_manager_set_show_local_greeter (self, g_value_get_boolean (value));
                break;
        default:
...
static void
scdm_manager_get_property (GObject    *object,
                          guint       prop_id,
                          GValue     *value,
                          GParamSpec *pspec)
{
...
static GObject *
scdm_manager_constructor (GType                  type,
                         guint                  n_construct_properties,
                         GObjectConstructParam *construct_properties)
{
        ScdmManager      *manager;

        manager = SCDM_MANAGER (G_OBJECT_CLASS (scdm_manager_parent_class)->constructor (type,
                                                                                       n_construct_properties,
                                                                                       construct_properties));

        scdm_dbus_manager_set_version (SCDM_DBUS_MANAGER (manager), PACKAGE_VERSION);

        manager->priv->local_factory = scdm_local_display_factory_new (manager->priv->display_store);
...
static void
scdm_manager_class_init (ScdmManagerClass *klass)
{
        GObjectClass   *object_class = G_OBJECT_CLASS (klass);

        object_class->get_property = scdm_manager_get_property;
        object_class->set_property = scdm_manager_set_property;
        object_class->constructor = scdm_manager_constructor;
        object_class->dispose = scdm_manager_dispose;

        signals [DISPLAY_ADDED] =
                g_signal_new ("display-added",
                              G_TYPE_FROM_CLASS (object_class),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (ScdmManagerClass, display_added),
                              NULL,
                              NULL,
        ...
        signals [DISPLAY_REMOVED] =
                g_signal_new ("display-removed",
                              G_TYPE_FROM_CLASS (object_class),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (ScdmManagerClass, display_removed),
                              NULL,
                              NULL,
...
static void
scdm_manager_init (ScdmManager *manager)
{

        manager->priv = SCDM_MANAGER_GET_PRIVATE (manager);

        manager->priv->display_store = scdm_display_store_new ();
...
static void
scdm_manager_dispose (GObject *object)
{
        ScdmManager *manager;
        ...
        if (!g_dbus_connection_is_closed (manager->priv->connection)) {
                scdm_display_store_foreach (manager->priv->display_store,
                                           (ScdmDisplayStoreFunc)unexport_display,
                                           manager);
                g_dbus_interface_skeleton_unexport (G_DBUS_INTERFACE_SKELETON (manager));
        }

        scdm_display_store_foreach (manager->priv->display_store,
                                   (ScdmDisplayStoreFunc) finish_display,
                                   manager);

        scdm_display_store_clear (manager->priv->display_store);
        ...
        G_OBJECT_CLASS (scdm_manager_parent_class)->dispose (object);
}

ScdmManager *
scdm_manager_new (void)
{
        if (manager_object != NULL) {
                ...
                manager_object = g_object_new (SCDM_TYPE_MANAGER, NULL);
        ...
        return SCDM_MANAGER (manager_object);
}
