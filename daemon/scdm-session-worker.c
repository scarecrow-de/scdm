...
#if defined (HAVE_ADT)
#include "scdm-session-solaris-auditor.h"
...
#define SCDM_SESSION_WORKER_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), SCDM_TYPE_SESSION_WORKER, ScdmSessionWorkerPrivate))

#define SCDM_SESSION_DBUS_PATH         "/io/github/scarecrow_de/DisplayManager/Session"
#define SCDM_SESSION_DBUS_NAME         "io.github.scarecrow_de.DisplayManager.Session"
...
#ifndef SCDM_PASSWD_AUXILLARY_BUFFER_SIZE
#define SCDM_PASSWD_AUXILLARY_BUFFER_SIZE 1024
#endif

#ifndef SCDM_SESSION_DEFAULT_PATH
#define SCDM_SESSION_DEFAULT_PATH "/usr/local/bin:/usr/bin:/bin"
#endif

#ifndef SCDM_SESSION_ROOT_UID
#define SCDM_SESSION_ROOT_UID 0
#endif

#ifndef SCDM_SESSION_LOG_FILENAME
#define SCDM_SESSION_LOG_FILENAME "session.log"
#endif
...
#ifdef SUPPORTS_PAM_EXTENSIONS
static char scdm_pam_extension_environment_block[_POSIX_ARG_MAX];

static const char * const
scdm_supported_pam_extensions[] = {
        SCDM_PAM_EXTENSION_CHOICE_LIST,
        NULL
};
#endif
...
static void     scdm_session_worker_class_init   (ScdmSessionWorkerClass *klass);
static void     scdm_session_worker_init         (ScdmSessionWorker      *session_worker);
static void     scdm_session_worker_finalize     (GObject               *object);

static void     scdm_session_worker_set_environment_variable (ScdmSessionWorker *worker,
                                                             const char       *key,
                                                             const char       *value);
...
G_DEFINE_TYPE_WITH_CODE (ScdmSessionWorker,
                         scdm_session_worker,
                         SCDM_DBUS_TYPE_WORKER_SKELETON,
                         G_IMPLEMENT_INTERFACE (SCDM_DBUS_TYPE_WORKER,
...
static gint
scdm_session_execute (const char *file,
                     char      **argv,
                     char      **envp,
                     gboolean    search_path)
{
 ...
 */
static gboolean
scdm_session_worker_get_username (ScdmSessionWorker  *worker,
                                 char             **username)
{
        gconstpointer item;
        ...
        if (pam_get_item (worker->priv->pam_handle, PAM_USER, &item) == PAM_SUCCESS) {
                ...
                if (worker->priv->auditor != NULL) {
                        scdm_session_auditor_set_username (worker->priv->auditor, (char *)item);
                }
                               ...
                               const char       *username)
{
        ...
        scdm_session_settings_load (worker->priv->user_settings,
                                   username);
}

static void
scdm_session_worker_update_username (ScdmSessionWorker *worker)
{
        char    *username;
        gboolean res;

        username = NULL;
        res = scdm_session_worker_get_username (worker, &username);
        if (res) {
                ...
                scdm_session_auditor_set_username (worker->priv->auditor, worker->priv->username);
                ...
                scdm_dbus_worker_emit_username_changed (SCDM_DBUS_WORKER (worker),
                ...
                if (worker->priv->username != NULL &&
                    ...
                    !scdm_session_settings_is_loaded (worker->priv->user_settings)) {
...
static gboolean
scdm_session_worker_ask_question (ScdmSessionWorker *worker,
                                 const char       *question,
                                 char            **answerp)
{
        return scdm_dbus_worker_manager_call_info_query_sync (worker->priv->manager,
                                                             worker->priv->service,
                                                             question,
                                                             answerp,
                                                             NULL,
...
static gboolean
scdm_session_worker_ask_for_secret (ScdmSessionWorker *worker,
                                   const char       *question,
                                   char            **answerp)
{
        return scdm_dbus_worker_manager_call_secret_info_query_sync (worker->priv->manager,
                                                                    worker->priv->service,
                                                                    question,
                                                                    answerp,
                                                                    NULL,
...
static gboolean
scdm_session_worker_report_info (ScdmSessionWorker *worker,
                                const char       *info)
{
        return scdm_dbus_worker_manager_call_info_sync (worker->priv->manager,
                                                       worker->priv->service,
                                                       info,
                                                       NULL,
                                                       NULL);
...
static gboolean
scdm_session_worker_report_problem (ScdmSessionWorker *worker,
                                   const char       *problem)
{
        return scdm_dbus_worker_manager_call_problem_sync (worker->priv->manager,
                                                          worker->priv->service,
                                                          problem,
                                                          NULL,
                                                          NULL);
...
#ifdef SUPPORTS_PAM_EXTENSIONS
static gboolean
scdm_session_worker_ask_list_of_choices (ScdmSessionWorker *worker,
                                        ...
                                        char            **answerp)
{
        ...
        res = scdm_dbus_worker_manager_call_choice_list_query_sync (worker->priv->manager,
                                                                    worker->priv->service,
                                                                    prompt_message,
                                                                    choices_as_variant,
                                                                    answerp,
...
static gboolean
scdm_session_worker_process_choice_list_request (ScdmSessionWorker                   *worker,
                                                ScdmPamExtensionChoiceListRequest  *request,
                                                ScdmPamExtensionChoiceListResponse *response)
{
        return scdm_session_worker_ask_list_of_choices (worker, request->prompt_message, &request->list, &response->key);
}

static gboolean
scdm_session_worker_process_extended_pam_message (ScdmSessionWorker          *worker,
                                                 ...
                                                 char                     **response)
{
        ...
        extended_message = SCDM_PAM_EXTENSION_MESSAGE_FROM_PAM_MESSAGE (query);

        if (SCDM_PAM_EXTENSION_MESSAGE_TRUNCATED (extended_message)) {
                g_warning ("PAM service requested binary response for truncated query");
                return FALSE;
        }

        if (SCDM_PAM_EXTENSION_MESSAGE_INVALID_TYPE (extended_message)) {
                g_warning ("PAM service requested binary response for unadvertised query type");
                return FALSE;
        }

        if (SCDM_PAM_EXTENSION_MESSAGE_MATCH (extended_message, worker->priv->extensions, SCDM_PAM_EXTENSION_CHOICE_LIST)) {
                ScdmPamExtensionChoiceListRequest *list_request = (ScdmPamExtensionChoiceListRequest *) extended_message;
                ScdmPamExtensionChoiceListResponse *list_response = malloc (SCDM_PAM_EXTENSION_CHOICE_LIST_RESPONSE_SIZE);

                g_debug ("ScdmSessionWorker: received extended pam message '%s'", SCDM_PAM_EXTENSION_CHOICE_LIST);

                SCDM_PAM_EXTENSION_CHOICE_LIST_RESPONSE_INIT (list_response);

                res = scdm_session_worker_process_choice_list_request (worker, list_request, list_response);
                ...
                *response = SCDM_PAM_EXTENSION_MESSAGE_TO_PAM_REPLY (list_response);
                return TRUE;
        } else {
...
static gboolean
scdm_session_worker_process_pam_message (ScdmSessionWorker          *worker,
                                        const struct pam_message  *query,
                                        char                     **response)
{
        char    *user_answer;
        ...
        scdm_session_worker_update_username (worker);

#ifdef SUPPORTS_PAM_EXTENSIONS
        if (query->msg_style == PAM_BINARY_PROMPT)
                return scdm_session_worker_process_extended_pam_message (worker, query, response);
#endif
        ...
        case PAM_PROMPT_ECHO_ON:
                res = scdm_session_worker_ask_question (worker, utf8_msg, &user_answer);
                break;
        case PAM_PROMPT_ECHO_OFF:
                res = scdm_session_worker_ask_for_secret (worker, utf8_msg, &user_answer);
                break;
        case PAM_TEXT_INFO:
                res = scdm_session_worker_report_info (worker, utf8_msg);
                break;
        case PAM_ERROR_MSG:
                res = scdm_session_worker_report_problem (worker, utf8_msg);
                break;
#ifdef PAM_RADIO_TYPE
        case PAM_RADIO_TYPE:
                msg = g_strdup_printf ("%s (yes/no)", utf8_msg);
                res = scdm_session_worker_ask_question (worker, msg, &user_answer);
                g_free (msg);
                break;
#endif
        default:
        ...
        if (worker->priv->timed_out) {
                scdm_dbus_worker_emit_cancel_pending_query (SCDM_DBUS_WORKER (worker));
                worker->priv->timed_out = FALSE;
        }
...
static int
scdm_session_worker_pam_new_messages_handler (int                        number_of_messages,
                                             ...
                                             ScdmSessionWorker          *worker)
{
        ...
        for (i = 0; i < number_of_messages; i++) {
                ...
                response = NULL;
                got_response = scdm_session_worker_process_pam_message (worker,
                                                                       messages[i],
                                                                       &response);
                if (!got_response) {
                        goto out;
...
static void
scdm_session_worker_start_auditor (ScdmSessionWorker *worker)
{
    /* Use dummy auditor so program session doesn't pollute user audit logs
     */
    if (worker->priv->is_program_session) {
            worker->priv->auditor = scdm_session_auditor_new (worker->priv->hostname,
 ...
 */
#if defined (HAVE_ADT)
        worker->priv->auditor = scdm_session_solaris_auditor_new (worker->priv->hostname,
                                                                 worker->priv->display_device);
#elif defined (HAVE_LIBAUDIT)
        worker->priv->auditor = scdm_session_linux_auditor_new (worker->priv->hostname,
                                                               worker->priv->display_device);
#else
        worker->priv->auditor = scdm_session_auditor_new (worker->priv->hostname,
                                                         worker->priv->display_device);
...
static void
scdm_session_worker_stop_auditor (ScdmSessionWorker *worker)
{
...
static void
scdm_session_worker_set_state (ScdmSessionWorker      *worker,
                              ScdmSessionWorkerState  state)
{
...
static void
scdm_session_worker_uninitialize_pam (ScdmSessionWorker *worker,
                                     int               status)
{
        ...
        scdm_session_worker_get_username (worker, NULL);

        if (worker->priv->state >= SCDM_SESSION_WORKER_STATE_SESSION_OPENED) {
                pam_close_session (worker->priv->pam_handle, 0);
                scdm_session_auditor_report_logout (worker->priv->auditor);
        } else {
                scdm_session_auditor_report_login_failure (worker->priv->auditor,
                                                          status,
                                                          pam_strerror (worker->priv->pam_handle, status));
        }

        if (worker->priv->state >= SCDM_SESSION_WORKER_STATE_ACCREDITED) {
        ...
        scdm_session_worker_stop_auditor (worker);
        ...
        if (g_strcmp0 (worker->priv->display_seat_id, "seat0") == 0) {
                ...
                if (worker->priv->session_vt != SCDM_INITIAL_VT) {
                        jump_to_vt (worker, SCDM_INITIAL_VT);
                }
        }
#endif
        ...
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_NONE);
}
...
static gboolean
scdm_session_worker_initialize_pam (ScdmSessionWorker   *worker,
                                   const char         *service,
                                   ...
                                   GError            **error)
{
...
#ifdef SUPPORTS_PAM_EXTENSIONS
        if (extensions != NULL) {
                SCDM_PAM_EXTENSION_ADVERTISE_SUPPORTED_EXTENSIONS (scdm_pam_extension_environment_block, extensions);
        }
#endif

        pam_conversation.conv = (ScdmSessionWorkerPamNewMessagesFunc) scdm_session_worker_pam_new_messages_handler;
        pam_conversation.appdata_ptr = worker;

        scdm_session_worker_start_auditor (worker);
        ...
        if (username == NULL) {
                ...
                if (error_code != PAM_SUCCESS) {
                        ...
                        g_set_error (error,
                                     SCDM_SESSION_WORKER_ERROR,
                                     SCDM_SESSION_WORKER_ERROR_AUTHENTICATING,
                                     "%s", "");
                        goto out;
                }
        }
        ...
        if (seat_id != NULL && seat_id[0] != '\0') {
                scdm_session_worker_set_environment_variable (worker, "XDG_SEAT", seat_id);
        }

        if (strcmp (service, "scdm-launch-environment") == 0) {
                scdm_session_worker_set_environment_variable (worker, "XDG_SESSION_CLASS", "greeter");
        }

        g_debug ("ScdmSessionWorker: state SETUP_COMPLETE");
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_SETUP_COMPLETE);

        /* Temporarily set PAM_TTY with the login VT,
           PAM_TTY will be reset with the users VT right before the user session is opened */
        g_snprintf (tty_string, 256, "/dev/tty%d", SCDM_INITIAL_VT);
 ...
 out:
        if (error_code != PAM_SUCCESS) {
                scdm_session_worker_uninitialize_pam (worker, error_code);
                return FALSE;
        }
...
static gboolean
scdm_session_worker_authenticate_user (ScdmSessionWorker *worker,
                                      gboolean          password_is_required,
                                      GError          **error)
{
        int error_code;
        ...
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_AUTHENTICATED);
...
static gboolean
scdm_session_worker_authorize_user (ScdmSessionWorker *worker,
                                   gboolean          password_is_required,
                                   GError          **error)
{
        int error_code;
        ...
        if (error_code == PAM_NEW_AUTHTOK_REQD && !worker->priv->is_program_session) {
                ...
                if (error_code != PAM_SUCCESS) {
                        scdm_session_auditor_report_password_change_failure (worker->priv->auditor);
                } else {
                        scdm_session_auditor_report_password_changed (worker->priv->auditor);
                }
        }
        ...
        if (error_code != PAM_SUCCESS) {
                ...
                g_set_error (error,
                             SCDM_SESSION_WORKER_ERROR,
                             SCDM_SESSION_WORKER_ERROR_AUTHORIZING,
                             "%s", get_friendly_error_message (error_code));
                goto out;
        }

        g_debug ("ScdmSessionWorker: state AUTHORIZED");
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_AUTHORIZED);
...
static void
scdm_session_worker_set_environment_variable (ScdmSessionWorker *worker,
                                             const char       *key,
                                             const char       *value)
{
        int error_code;
...
static char *
scdm_session_worker_get_environment_variable (ScdmSessionWorker *worker,
                                             const char       *key)
{
...
static void
scdm_session_worker_update_environment_from_passwd_info (ScdmSessionWorker *worker,
                                                        uid_t             uid,
                                                        gid_t             gid,
                                                        const char       *home,
                                                        const char       *shell)
{
        scdm_session_worker_set_environment_variable (worker, "LOGNAME", worker->priv->username);
        scdm_session_worker_set_environment_variable (worker, "USER", worker->priv->username);
        scdm_session_worker_set_environment_variable (worker, "USERNAME", worker->priv->username);
        scdm_session_worker_set_environment_variable (worker, "HOME", home);
        scdm_session_worker_set_environment_variable (worker, "PWD", home);
        scdm_session_worker_set_environment_variable (worker, "SHELL", shell);
}

static gboolean
scdm_session_worker_environment_variable_is_set (ScdmSessionWorker *worker,
                                                const char       *key)
{
              ...
              gid_t              gid)
{
         ...
         */
        if (setreuid (uid, SCDM_SESSION_ROOT_UID) < 0) {
                return FALSE;
        }
#endif
        worker->priv->uid = uid;
                     ...
                     char      **shellp)
{
        ...
        if (required_aux_buffer_size < 0) {
                aux_buffer_size = SCDM_PASSWD_AUXILLARY_BUFFER_SIZE;
        } else {
...
get_var_cb (const char *key,
            gpointer user_data)
{
        return scdm_session_worker_get_environment_variable (user_data, key);
}
        ...
        if (g_file_load_contents (file, NULL, &contents, NULL, NULL, NULL)) {
                ...
                for (i = 0; lines[i] != NULL; i++) {
                        ...
                        if (*p == '#' || *p == '\0')
                                continue;
                        var = p;
                        while (scdm_shell_var_is_valid_char (*p, p == var))
                                p++;
                        var_end = p;
                        while (g_ascii_isspace (*p))
                                p++;
                        ...
                        *var_end = 0;
                        ...
                        expanded = scdm_shell_expand (p, get_var_cb, worker);
                        expanded = g_strchomp (expanded);
                        scdm_session_worker_set_environment_variable (worker, var, expanded);
                        g_free (expanded);
                }
                g_strfreev (lines);
        }
...
static void
scdm_session_worker_load_env_dir (ScdmSessionWorker *worker,
                                 GFile *dir)
{
        GFileInfo *info = NULL;
...
static void
scdm_session_worker_load_env_d (ScdmSessionWorker *worker)
{
        GFile *dir;

        dir = g_file_new_for_path (DATADIR "/scdm/env.d");
        scdm_session_worker_load_env_dir (worker, dir);
        g_object_unref (dir);
...
static gboolean
scdm_session_worker_accredit_user (ScdmSessionWorker  *worker,
                                  GError           **error)
{
        gboolean ret;
        gboolean res;
        ...
        if (worker->priv->username == NULL) {
                ...
                g_set_error (error,
                             SCDM_SESSION_WORKER_ERROR,
                             SCDM_SESSION_WORKER_ERROR_GIVING_CREDENTIALS,
        ...
        scdm_session_worker_update_environment_from_passwd_info (worker,
                                                                uid,
                                                                gid,
                                                                home,
                                                                shell);
         ...
         */
        if (!scdm_session_worker_environment_variable_is_set (worker, "PATH")) {
                if (strcmp (BINDIR, "/usr/bin") == 0) {
                        scdm_session_worker_set_environment_variable (worker, "PATH",
                                                                     SCDM_SESSION_DEFAULT_PATH);
                } else {
                        scdm_session_worker_set_environment_variable (worker, "PATH",
                                                                     BINDIR ":" SCDM_SESSION_DEFAULT_PATH);
                }
        }
        ...
        if (! _change_user (worker, uid, gid)) {
                ...
                g_set_error (error, SCDM_SESSION_WORKER_ERROR,
                             SCDM_SESSION_WORKER_ERROR_GIVING_CREDENTIALS,
 ...
 out:
        ...
        if (ret) {
                ...
                scdm_session_worker_get_username (worker, NULL);
                scdm_session_auditor_report_user_accredited (worker->priv->auditor);
                scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_ACCREDITED);
        } else {
                scdm_session_worker_uninitialize_pam (worker, error_code);
        }
...
static const char * const *
scdm_session_worker_get_environment (ScdmSessionWorker *worker)
{
                            ...
                            ScdmSessionWorker *worker)
{
        ...
        scdm_session_worker_uninitialize_pam (worker, PAM_SUCCESS);

        scdm_dbus_worker_emit_session_exited (SCDM_DBUS_WORKER (worker),
                                             worker->priv->service,
                                             status);
...
static void
scdm_session_worker_watch_child (ScdmSessionWorker *worker)
{
...
_open_user_session_log (const char *dir)
{
        ...
        filename = g_build_filename (dir, SCDM_SESSION_LOG_FILENAME, NULL);
...
static gboolean
scdm_session_worker_start_session (ScdmSessionWorker  *worker,
                                  GError           **error)
{
        struct passwd *passwd_entry;
        pid_t session_pid;
        ...
        scdm_get_pwent_for_name (worker->priv->username, &passwd_entry);
        if (worker->priv->is_program_session) {
        ...
        if (g_strcmp0 (worker->priv->display_seat_id, "seat0") == 0) {
                if (worker->priv->display_mode == SCDM_SESSION_DISPLAY_MODE_NEW_VT) {
        ...
        if (!worker->priv->is_program_session && !run_script (worker, GDMCONFDIR "/PostLogin")) {
                g_set_error (error,
                             SCDM_SESSION_WORKER_ERROR,
                             SCDM_SESSION_WORKER_ERROR_OPENING_SESSION,
        ...
        if (session_pid == 0) {
                ...
                if (!worker->priv->is_program_session) {
                        scdm_session_worker_load_env_d (worker);
                }

                environment = scdm_session_worker_get_environment (worker);

                g_assert (geteuid () == getuid ());

                home_dir = scdm_session_worker_get_environment_variable (worker, "HOME");
                ...
                if (has_journald) {
                         ...
                         */
                        scdm_clear_close_on_exec_flag (stdout_fd);
                        scdm_clear_close_on_exec_flag (stderr_fd);
                }
#endif
                if (!has_journald && !worker->priv->is_program_session) {
                        if (home_dir != NULL && home_dir[0] != '\0') {
                                ...
                                cache_dir = scdm_session_worker_get_environment_variable (worker, "XDG_CACHE_HOME");
                                if (cache_dir == NULL || cache_dir[0] == '\0') {
                                        cache_dir = g_build_filename (home_dir, ".cache", NULL);
                                }

                                log_dir = g_build_filename (cache_dir, "scdm", NULL);
                                g_free (cache_dir);
                ...
                scdm_session_execute (worker->priv->arguments[0],
        ...
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_SESSION_STARTED);

        scdm_session_worker_watch_child (worker);
...
set_up_for_new_vt (ScdmSessionWorker *worker)
{
         ...
         * by VT_OPENQRY
         * */
        g_snprintf (tty_string, sizeof (tty_string), "/dev/tty%d", SCDM_INITIAL_VT);
        ...
        scdm_session_worker_set_environment_variable (worker,
                                                     "XDG_VTNR",
                                                     vt_string);
...
static gboolean
scdm_session_worker_open_session (ScdmSessionWorker  *worker,
                                 GError           **error)
{
        int error_code;
        int flags;
        ...
        g_assert (worker->priv->state == SCDM_SESSION_WORKER_STATE_ACCOUNT_DETAILS_SAVED);
        g_assert (geteuid () == 0);
        ...
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_SESSION_OPENED);

        session_id = scdm_session_worker_get_environment_variable (worker, "XDG_SESSION_ID");
 ...
 out:
        ...
        scdm_session_worker_get_username (worker, NULL);
        scdm_session_auditor_report_login (worker->priv->auditor);

        return TRUE;
}

static void
scdm_session_worker_set_server_address (ScdmSessionWorker *worker,
                                       const char       *address)
{
...
static void
scdm_session_worker_set_is_reauth_session (ScdmSessionWorker *worker,
...
static void
scdm_session_worker_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
        ...
        self = SCDM_SESSION_WORKER (object);

        switch (prop_id) {
        case PROP_SERVER_ADDRESS:
                scdm_session_worker_set_server_address (self, g_value_get_string (value));
                break;
        case PROP_IS_REAUTH_SESSION:
                scdm_session_worker_set_is_reauth_session (self, g_value_get_boolean (value));
                break;
        default:
...
static void
scdm_session_worker_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
...
static gboolean
scdm_session_worker_handle_set_environment_variable (ScdmDBusWorker         *object,
                                                    GDBusMethodInvocation *invocation,
                                                    const char            *key,
                                                    const char            *value)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        scdm_session_worker_set_environment_variable (worker, key, value);
        scdm_dbus_worker_complete_set_environment_variable (object, invocation);
        return TRUE;
}

static gboolean
scdm_session_worker_handle_set_session_name (ScdmDBusWorker         *object,
                                            GDBusMethodInvocation *invocation,
                                            const char            *session_name)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        g_debug ("ScdmSessionWorker: session name set to %s", session_name);
        scdm_session_settings_set_session_name (worker->priv->user_settings,
                                               session_name);
        scdm_dbus_worker_complete_set_session_name (object, invocation);
        return TRUE;
}

static gboolean
scdm_session_worker_handle_set_session_display_mode (ScdmDBusWorker         *object,
                                                    GDBusMethodInvocation *invocation,
                                                    const char            *str)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        g_debug ("ScdmSessionWorker: session display mode set to %s", str);
        worker->priv->display_mode = scdm_session_display_mode_from_string (str);
        scdm_dbus_worker_complete_set_session_display_mode (object, invocation);
        return TRUE;
}

static gboolean
scdm_session_worker_handle_set_language_name (ScdmDBusWorker         *object,
                                             GDBusMethodInvocation *invocation,
                                             const char            *language_name)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        g_debug ("ScdmSessionWorker: language name set to %s", language_name);
        scdm_session_settings_set_language_name (worker->priv->user_settings,
                                                language_name);
        scdm_dbus_worker_complete_set_language_name (object, invocation);
        return TRUE;
}
...
on_saved_language_name_read (ScdmSessionWorker *worker)
{
        ...
        language_name = scdm_session_settings_get_language_name (worker->priv->user_settings);

        g_debug ("ScdmSessionWorker: Saved language is %s", language_name);
        scdm_dbus_worker_emit_saved_language_name_read (SCDM_DBUS_WORKER (worker),
                                                       language_name);
...
on_saved_session_name_read (ScdmSessionWorker *worker)
{
        ...
        session_name = scdm_session_settings_get_session_name (worker->priv->user_settings);

        g_debug ("ScdmSessionWorker: Saved session is %s", session_name);
        scdm_dbus_worker_emit_saved_session_name_read (SCDM_DBUS_WORKER (worker),
                                                      session_name);
        g_free (session_name);
}
...
do_setup (ScdmSessionWorker *worker)
{
        ...
        error = NULL;
        res = scdm_session_worker_initialize_pam (worker,
                                                 worker->priv->service,
...
do_authenticate (ScdmSessionWorker *worker)
{
         ...
         */
        error = NULL;
        res = scdm_session_worker_authenticate_user (worker,
        ...
        if (res) {
                ...
                scdm_dbus_worker_complete_authenticate (SCDM_DBUS_WORKER (worker), worker->priv->pending_invocation);
        } else {
...
do_authorize (ScdmSessionWorker *worker)
{
         ...
         */
        error = NULL;
        res = scdm_session_worker_authorize_user (worker,
                                                 worker->priv->password_is_required,
                                                 &error);
        if (res) {
                scdm_dbus_worker_complete_authorize (SCDM_DBUS_WORKER (worker), worker->priv->pending_invocation);
        } else {
                g_dbus_method_invocation_take_error (worker->priv->pending_invocation, error);
        }
        worker->priv->pending_invocation = NULL;
...
do_accredit (ScdmSessionWorker *worker)
{
         ...
         */
        error = NULL;
        res = scdm_session_worker_accredit_user (worker, &error);

        if (res) {
                scdm_dbus_worker_complete_establish_credentials (SCDM_DBUS_WORKER (worker), worker->priv->pending_invocation);
        } else {
                g_dbus_method_invocation_take_error (worker->priv->pending_invocation, error);
        }
        worker->priv->pending_invocation = NULL;
...
{
        g_assert (worker->priv->state == SCDM_SESSION_WORKER_STATE_ACCREDITED);

        g_debug ("ScdmSessionWorker: saving account details for user %s", worker->priv->username);
        scdm_session_worker_set_state (worker, SCDM_SESSION_WORKER_STATE_ACCOUNT_DETAILS_SAVED);
        if (!scdm_session_settings_save (worker->priv->user_settings,
                                        worker->priv->username)) {
...
on_settings_is_loaded_changed (ScdmSessionSettings *user_settings,
                               ...
                               ScdmSessionWorker   *worker)
{
        if (!scdm_session_settings_is_loaded (worker->priv->user_settings)) {
                return;
        }

        /* These signal handlers should be disconnected after the loading,
         * so that scdm_session_settings_set_* APIs don't cause the emitting
        ...
        if (worker->priv->state == SCDM_SESSION_WORKER_STATE_NONE) {
                g_debug ("ScdmSessionWorker: queuing setup for user: %s %s",
                         worker->priv->username, worker->priv->display_device);
                queue_state_change (worker);
        } else if (worker->priv->state == SCDM_SESSION_WORKER_STATE_ACCREDITED) {
...
do_open_session (ScdmSessionWorker *worker)
{
        ...
        error = NULL;
        res = scdm_session_worker_open_session (worker, &error);
        ...
        if (res) {
                ...
                scdm_dbus_worker_complete_open (SCDM_DBUS_WORKER (worker), worker->priv->pending_invocation, session_id);
        } else {
                g_dbus_method_invocation_take_error (worker->priv->pending_invocation, error);
        }
        worker->priv->pending_invocation = NULL;
...
do_start_session (ScdmSessionWorker *worker)
{
        ...
        error = NULL;
        res = scdm_session_worker_start_session (worker, &error);
        if (res) {
                scdm_dbus_worker_complete_start_program (SCDM_DBUS_WORKER (worker),
...
get_state_name (int state)
{
        ...
        switch (state) {
        case SCDM_SESSION_WORKER_STATE_NONE:
                name = "NONE";
                break;
        case SCDM_SESSION_WORKER_STATE_SETUP_COMPLETE:
                name = "SETUP_COMPLETE";
                break;
        case SCDM_SESSION_WORKER_STATE_AUTHENTICATED:
                name = "AUTHENTICATED";
                break;
        case SCDM_SESSION_WORKER_STATE_AUTHORIZED:
                name = "AUTHORIZED";
                break;
        case SCDM_SESSION_WORKER_STATE_ACCREDITED:
                name = "ACCREDITED";
                break;
        case SCDM_SESSION_WORKER_STATE_ACCOUNT_DETAILS_SAVED:
                name = "ACCOUNT_DETAILS_SAVED";
                break;
        case SCDM_SESSION_WORKER_STATE_SESSION_OPENED:
                name = "SESSION_OPENED";
                break;
        case SCDM_SESSION_WORKER_STATE_SESSION_STARTED:
                       ...
                       int                    new_state)
{
        if (worker->priv->pending_invocation != NULL) {
                g_dbus_method_invocation_return_error (invocation,
                                                       SCDM_SESSION_WORKER_ERROR,
                                                       SCDM_SESSION_WORKER_ERROR_OUTSTANDING_REQUEST,
                ...
                g_dbus_method_invocation_return_error (invocation,
                                                       SCDM_SESSION_WORKER_ERROR,
                                                       SCDM_SESSION_WORKER_ERROR_WRONG_STATE,
...
static gboolean
scdm_session_worker_handle_authenticate (ScdmDBusWorker         *object,
                                        GDBusMethodInvocation *invocation)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        validate_and_queue_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_AUTHENTICATED);
        return TRUE;
}

static gboolean
scdm_session_worker_handle_authorize (ScdmDBusWorker         *object,
                                     GDBusMethodInvocation *invocation)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        validate_and_queue_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_AUTHORIZED);
        return TRUE;
}

static gboolean
scdm_session_worker_handle_establish_credentials (ScdmDBusWorker         *object,
                                                 GDBusMethodInvocation *invocation)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        validate_and_queue_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_ACCREDITED);
...
static gboolean
scdm_session_worker_handle_open (ScdmDBusWorker         *object,
                                GDBusMethodInvocation *invocation)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        validate_and_queue_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_ACCOUNT_DETAILS_SAVED);
        return TRUE;
}
...
filter_extensions (const char * const *extensions)
{
        ...
        for (i = 0; extensions[i] != NULL; i++) {
                for (j = 0; scdm_supported_pam_extensions[j] != NULL; j++) {
                        if (g_strcmp0 (extensions[i], scdm_supported_pam_extensions[j]) == 0) {
                                g_ptr_array_add (array, g_strdup (scdm_supported_pam_extensions[j]));
                                break;
                        }
                }
        }
...
static gboolean
scdm_session_worker_handle_initialize (ScdmDBusWorker         *object,
                                      GDBusMethodInvocation *invocation,
                                      GVariant              *details)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        ...
        if (!validate_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_SETUP_COMPLETE))
                return TRUE;
        ...
        if (!worker->priv->is_program_session) {
                ...
                if (worker->priv->username) {
                        wait_for_settings = !scdm_session_settings_load (worker->priv->user_settings,
                                                                        worker->priv->username);
                }
        }
...
static gboolean
scdm_session_worker_handle_setup (ScdmDBusWorker         *object,
                                 GDBusMethodInvocation *invocation,
                                 const char            *service,
                                 const char            *x11_display_name,
                                 const char            *x11_authority_file,
...
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        validate_and_queue_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_SETUP_COMPLETE);
...
static gboolean
scdm_session_worker_handle_setup_for_user (ScdmDBusWorker         *object,
                                          GDBusMethodInvocation *invocation,
                                          const char            *service,
                                          const char            *username,
                                          const char            *x11_display_name,
                                          ...
                                          gboolean               display_is_initial)
{
         ...
         */
        worker->priv->pending_invocation = invocation;
        if (scdm_session_settings_load (worker->priv->user_settings, username)) {
                queue_state_change (worker);
        } else {
                g_signal_connect (G_OBJECT (worker->priv->user_settings),
                                  "notify::is-loaded",
...
static gboolean
scdm_session_worker_handle_setup_for_program (ScdmDBusWorker         *object,
                                             GDBusMethodInvocation *invocation,
                                             const char            *service,
                                             const char            *username,
                                             const char            *x11_display_name,
...
static gboolean
scdm_session_worker_handle_start_program (ScdmDBusWorker         *object,
                                         GDBusMethodInvocation *invocation,
                                         const char            *text)
{
        ScdmSessionWorker *worker = SCDM_SESSION_WORKER (object);
        GError *parse_error = NULL;
        validate_state_change (worker, invocation, SCDM_SESSION_WORKER_STATE_SESSION_STARTED);

        if (worker->priv->is_reauth_session) {
                g_dbus_method_invocation_return_error (invocation,
                                                       SCDM_SESSION_WORKER_ERROR,
                                                       SCDM_SESSION_WORKER_ERROR_IN_REAUTH_SESSION,
                                           ...
                                           ReauthenticationRequest *request)
{
        ...
        scdm_dbus_worker_emit_reauthenticated (SCDM_DBUS_WORKER (worker), service_name);
}
                              ...
                              GDBusMethodInvocation *invocation)
{
        ...
        request->session = scdm_session_new (SCDM_SESSION_VERIFICATION_MODE_REAUTHENTICATE,
                                            uid_of_caller,
                                            worker->priv->x11_display_name,
                                            worker->priv->hostname,
                                            worker->priv->display_device,
        ...
        address = scdm_session_get_server_address (request->session);

        scdm_dbus_worker_complete_start_reauthentication (SCDM_DBUS_WORKER (worker),
                                                         invocation,
                                                         address);
...
static gboolean
scdm_session_worker_handle_start_reauthentication (ScdmDBusWorker         *object,
                                                  GDBusMethodInvocation *invocation,
                                                  int                    pid_of_caller,
                                                  int                    uid_of_caller)
{
        ...
        if (worker->priv->state != SCDM_SESSION_WORKER_STATE_SESSION_STARTED) {
                g_dbus_method_invocation_return_error (invocation,
                                                       SCDM_SESSION_WORKER_ERROR,
                                                       SCDM_SESSION_WORKER_ERROR_WRONG_STATE,
...
static GObject *
scdm_session_worker_constructor (GType                  type,
                                ...
                                GObjectConstructParam *construct_properties)
{
        ...
        worker = SCDM_SESSION_WORKER (G_OBJECT_CLASS (scdm_session_worker_parent_class)->constructor (type,
        ...
        worker->priv->manager = SCDM_DBUS_WORKER_MANAGER (scdm_dbus_worker_manager_proxy_new_sync (worker->priv->connection,
                                                                                                 G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
                                                                                                 NULL, /* dbus name */
                                                                                                 SCDM_SESSION_DBUS_PATH,
                                                                                                 NULL,
                                                                                                 &error));
         ...
         */
        scdm_dbus_worker_manager_call_hello_sync (worker->priv->manager,
                                                 NULL,
                                                 NULL);
...
{
        interface->handle_initialize = scdm_session_worker_handle_initialize;
        /* The next three are for backward compat only */
        interface->handle_setup = scdm_session_worker_handle_setup;
        interface->handle_setup_for_user = scdm_session_worker_handle_setup_for_user;
        interface->handle_setup_for_program = scdm_session_worker_handle_setup_for_program;
        interface->handle_authenticate = scdm_session_worker_handle_authenticate;
        interface->handle_authorize = scdm_session_worker_handle_authorize;
        interface->handle_establish_credentials = scdm_session_worker_handle_establish_credentials;
        interface->handle_open = scdm_session_worker_handle_open;
        interface->handle_set_language_name = scdm_session_worker_handle_set_language_name;
        interface->handle_set_session_name = scdm_session_worker_handle_set_session_name;
        interface->handle_set_session_display_mode = scdm_session_worker_handle_set_session_display_mode;
        interface->handle_set_environment_variable = scdm_session_worker_handle_set_environment_variable;
        interface->handle_start_program = scdm_session_worker_handle_start_program;
        interface->handle_start_reauthentication = scdm_session_worker_handle_start_reauthentication;
}

static void
scdm_session_worker_class_init (ScdmSessionWorkerClass *klass)
{
        GObjectClass    *object_class = G_OBJECT_CLASS (klass);

        object_class->get_property = scdm_session_worker_get_property;
        object_class->set_property = scdm_session_worker_set_property;
        object_class->constructor = scdm_session_worker_constructor;
        object_class->finalize = scdm_session_worker_finalize;
        ...
        g_object_class_install_property (object_class,
                                         ...
                                         g_param_spec_enum ("state",
                                                            "state",
                                                            "state",
                                                            SCDM_TYPE_SESSION_WORKER_STATE,
                                                            SCDM_SESSION_WORKER_STATE_NONE,
...
static void
scdm_session_worker_init (ScdmSessionWorker *worker)
{
        worker->priv = SCDM_SESSION_WORKER_GET_PRIVATE (worker);

        worker->priv->user_settings = scdm_session_settings_new ();
...
static void
scdm_session_worker_unwatch_child (ScdmSessionWorker *worker)
{
...
static void
scdm_session_worker_finalize (GObject *object)
{
        ScdmSessionWorker *worker;
        ...
        scdm_session_worker_unwatch_child (worker);

        if (worker->priv->child_pid > 0) {
                scdm_signal_pid (worker->priv->child_pid, SIGTERM);
                scdm_wait_on_pid (worker->priv->child_pid);
        }
        ...
        G_OBJECT_CLASS (scdm_session_worker_parent_class)->finalize (object);
}

ScdmSessionWorker *
scdm_session_worker_new (const char *address,
                        gboolean    is_reauth_session)
{
        GObject *object;

        object = g_object_new (SCDM_TYPE_SESSION_WORKER,
                               "server-address", address,
                               "is-reauth-session", is_reauth_session,
                               NULL);

        return SCDM_SESSION_WORKER (object);
}
