...
struct _ScdmSession
{
        ...
        ScdmSessionConversation *session_conversation;

        char                 **conversation_environment;

        ScdmDBusUserVerifier   *user_verifier_interface;
        GHashTable            *user_verifier_extensions;
        ScdmDBusGreeter        *greeter_interface;
        ScdmDBusRemoteGreeter  *remote_greeter_interface;
        ScdmDBusChooser        *chooser_interface;
...
#ifdef ENABLE_WAYLAND_SUPPORT
static gboolean scdm_session_is_wayland_session (ScdmSession *self);
#endif
static void update_session_type (ScdmSession *self);
static void set_session_type (ScdmSession *self,
                              const char *session_type);
static void close_conversation (ScdmSessionConversation *conversation);

static guint signals [LAST_SIGNAL] = { 0, };

G_DEFINE_TYPE (ScdmSession,
               scdm_session,
               G_TYPE_OBJECT);

static ScdmSessionConversation *
find_conversation_by_name (ScdmSession *self,
...
static void
report_and_stop_conversation (ScdmSession *self,
                              const char *service_name,
                              GError     *error)
{
        ...
        if (self->user_verifier_interface != NULL) {
                if (g_error_matches (error,
                        ...
                        scdm_dbus_user_verifier_emit_problem (self->user_verifier_interface,
                                                             service_name,
                                                             error->message);
                }
                scdm_dbus_user_verifier_emit_verification_failed (self->user_verifier_interface,
                                                                 service_name);
        }

        scdm_session_stop_conversation (self, service_name);
}

static void
on_authenticate_cb (ScdmDBusWorker *proxy,
                    GAsyncResult  *res,
                    gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ScdmSession *self;
        char *service_name;
        ...
        worked = scdm_dbus_worker_call_authenticate_finish (proxy, res, &error);
        ...
        if (worked) {
                scdm_session_authorize (self, service_name);
        } else {
                g_signal_emit (self,
...
static void
on_authorize_cb (ScdmDBusWorker *proxy,
                 GAsyncResult  *res,
                 gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ...
        worked = scdm_dbus_worker_call_authorize_finish (proxy, res, &error);
        ...
        if (worked) {
                scdm_session_accredit (self, service_name);
        } else {
                report_and_stop_conversation (self, service_name, error);
        }
}
...
static void
on_establish_credentials_cb (ScdmDBusWorker *proxy,
                             GAsyncResult  *res,
                             gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ...
        worked = scdm_dbus_worker_call_establish_credentials_finish (proxy, res, &error);
        ...
        if (worked) {
                if (self->user_verifier_interface != NULL) {
                        scdm_dbus_user_verifier_emit_verification_complete (self->user_verifier_interface,
                                                                           service_name);
...
static char **
get_system_session_dirs (ScdmSession *self)
{
...
static GKeyFile *
load_key_file_for_file (ScdmSession   *self,
                        const char   *file,
                        char        **full_path)
{
        GKeyFile   *key_file;
        ...
        res = g_key_file_load_from_dirs (key_file,
                                         ...
                                         &error);
        if (! res) {
                g_debug ("ScdmSession: File '%s' not found: %s", file, error->message);
                g_error_free (error);
...
static gboolean
get_session_command_for_file (ScdmSession  *self,
                              const char  *file,
                              char       **command)
{
        GKeyFile   *key_file;
        ...
        g_debug ("ScdmSession: getting session command for file '%s'", file);
        ...
        res = g_key_file_get_boolean (key_file,
                                      ...
                                      &error);
        if (error == NULL && res) {
                g_debug ("ScdmSession: Session %s is marked as hidden", file);
                goto out;
        }
        ...
        if (exec != NULL) {
                ...
                if (! res) {
                        g_debug ("ScdmSession: Command not found: %s",
                                 G_KEY_FILE_DESKTOP_KEY_TRY_EXEC);
                        goto out;
                }
        }
        ...
        exec = g_key_file_get_string (key_file,
                                      ...
                                      &error);
        if (error != NULL) {
                g_debug ("ScdmSession: %s key not found: %s",
                         G_KEY_FILE_DESKTOP_KEY_EXEC,
                         error->message);
                g_error_free (error);
                goto out;
...
static gboolean
get_session_command_for_name (ScdmSession  *self,
                              const char  *name,
                              char       **command)
{
        gboolean res;
...
static const char *
get_default_language_name (ScdmSession *self)
{
...
static const char *
get_fallback_session_name (ScdmSession *self)
{
        char          **search_dirs;
        int             i;
        char           *name;
        ...
        if (g_sequence_iter_is_end (session))
                g_error ("ScdmSession: no session desktop files installed, aborting...");
...
static const char *
get_default_session_name (ScdmSession *self)
{
...
static void
scdm_session_defaults_changed (ScdmSession *self)
{

        update_session_type (self);

        if (self->greeter_interface != NULL) {
                scdm_dbus_greeter_emit_default_language_name_changed (self->greeter_interface,
                                                                     get_default_language_name (self));
                scdm_dbus_greeter_emit_default_session_name_changed (self->greeter_interface,
                                                                    get_default_session_name (self));
        }
}

void
scdm_session_select_user (ScdmSession *self,
                         const char *text)
{

        g_debug ("ScdmSession: selecting user '%s' for session '%s' (%p)",
                 text,
                 scdm_session_get_session_id (self),
                 self);
...
static void
cancel_pending_query (ScdmSessionConversation *conversation)
{
...
static void
answer_pending_query (ScdmSessionConversation *conversation,
                      const char             *answer)
{
...
static void
set_pending_query (ScdmSessionConversation *conversation,
...
static gboolean
scdm_session_handle_choice_list_query (ScdmDBusWorkerManager  *worker_manager_interface,
...
{
        ScdmSessionConversation *conversation;
        ScdmDBusUserVerifierChoiceList *choice_list_interface = NULL;

        g_debug ("ScdmSession: choice query for service '%s'", service_name);

        if (self->user_verifier_extensions != NULL)
                choice_list_interface = g_hash_table_lookup (self->user_verifier_extensions,
                                                             scdm_dbus_user_verifier_choice_list_interface_info ()->name);
        ...
        if (conversation != NULL) {
                ...
                g_debug ("ScdmSession: emitting choice query '%s'", prompt_message);
                scdm_dbus_user_verifier_choice_list_emit_choice_query (choice_list_interface,
                                                                      service_name,
                                                                      prompt_message,
                                                                      query);
        }
...
static gboolean
scdm_session_handle_info_query (ScdmDBusWorkerManager  *worker_manager_interface,
                               GDBusMethodInvocation *invocation,
                               const char            *service_name,
                               const char            *query,
                               ScdmSession            *self)
{
        ...
        if (conversation != NULL) {
                ...
                scdm_dbus_user_verifier_emit_info_query (self->user_verifier_interface,
                                                        service_name,
                                                        query);
        }
...
static gboolean
scdm_session_handle_secret_info_query (ScdmDBusWorkerManager  *worker_manager_interface,
                                      GDBusMethodInvocation *invocation,
                                      const char            *service_name,
                                      const char            *query,
                                      ScdmSession            *self)
{
        ...
        if (conversation != NULL) {
                ...
                scdm_dbus_user_verifier_emit_secret_info_query (self->user_verifier_interface,
                                                               service_name,
                                                               query);
        }
...
static gboolean
scdm_session_handle_info (ScdmDBusWorkerManager  *worker_manager_interface,
                         GDBusMethodInvocation *invocation,
                         const char            *service_name,
                         const char            *info,
                         ScdmSession            *self)
{
        scdm_dbus_worker_manager_complete_info (worker_manager_interface,
                                               invocation);

        if (self->user_verifier_interface != NULL) {
                scdm_dbus_user_verifier_emit_info (self->user_verifier_interface,
                                                  service_name,
                                                  info);
        }
...
static void
worker_on_cancel_pending_query (ScdmDBusWorker          *worker,
                                ScdmSessionConversation *conversation)
{
...
static gboolean
scdm_session_handle_problem (ScdmDBusWorkerManager  *worker_manager_interface,
                            GDBusMethodInvocation *invocation,
                            const char            *service_name,
                            const char            *problem,
                            ScdmSession            *self)
{
        scdm_dbus_worker_manager_complete_problem (worker_manager_interface,
                                                  invocation);
...
static void
on_opened (ScdmDBusWorker *worker,
           GAsyncResult  *res,
           gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ...
        worked = scdm_dbus_worker_call_open_finish (worker,
                                                   &session_id,
                                                   res,
                                                   &error);
        ...
        if (worked) {
                ...
                if (self->greeter_interface != NULL) {
                        scdm_dbus_greeter_emit_session_opened (self->greeter_interface,
                                                              service_name);
                }
                ...
                g_debug ("ScdmSession: Emitting 'session-opened' signal");
                g_signal_emit (self, signals[SESSION_OPENED], 0, service_name, session_id);
        } else {
                report_and_stop_conversation (self, service_name, error);

                g_debug ("ScdmSession: Emitting 'session-start-failed' signal");
                g_signal_emit (self, signals[SESSION_START_FAILED], 0, service_name, error->message);
        }
}

static void
worker_on_username_changed (ScdmDBusWorker          *worker,
                            const char             *username,
                            ScdmSessionConversation *conversation)
{
        ScdmSession *self = conversation->session;

        g_debug ("ScdmSession: changing username from '%s' to '%s'",
                 self->selected_user != NULL ? self->selected_user : "<unset>",
                 (strlen (username)) ? username : "<unset>");

        scdm_session_select_user (self, (strlen (username) > 0) ? g_strdup (username) : NULL);
        scdm_session_defaults_changed (self);
}

static void
worker_on_session_exited (ScdmDBusWorker          *worker,
                          const char             *service_name,
                          int                     status,
                          ScdmSessionConversation *conversation)
{
        ...
        if (WIFEXITED (status)) {
                g_debug ("ScdmSession: Emitting 'session-exited' signal with exit code '%d'",
                  WEXITSTATUS (status));
                g_signal_emit (self, signals[SESSION_EXITED], 0, WEXITSTATUS (status));
        } else if (WIFSIGNALED (status)) {
                g_debug ("ScdmSession: Emitting 'session-died' signal with signal number '%d'",
                  WTERMSIG (status));
...
static void
on_reauthentication_started_cb (ScdmDBusWorker *worker,
                                GAsyncResult  *res,
                                gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ...
        worked = scdm_dbus_worker_call_start_reauthentication_finish (worker,
                                                                     &address,
                                                                     res,
                                                                     &error);
        ...
        if (worked) {
                ...
                g_debug ("ScdmSession: Emitting 'reauthentication-started' signal for caller pid '%d'", pid_of_caller);
...
static void
worker_on_reauthenticated (ScdmDBusWorker          *worker,
                           const char             *service_name,
                           ScdmSessionConversation *conversation)
{
        ScdmSession *self = conversation->session;
        g_debug ("ScdmSession: Emitting 'reauthenticated' signal ");
        g_signal_emit (self, signals[REAUTHENTICATED], 0, service_name);
}

static void
worker_on_saved_language_name_read (ScdmDBusWorker          *worker,
...
static void
worker_on_saved_session_name_read (ScdmDBusWorker          *worker,
                                   ...
                                   ScdmSessionConversation *conversation)
{
        ...
        if (! get_session_command_for_name (self, session_name, NULL)) {
                ...
                g_debug ("ScdmSession: not using invalid .dmrc session: %s", session_name);
                g_free (self->saved_session);
                self->saved_session = NULL;
...
static ScdmSessionConversation *
find_conversation_by_pid (ScdmSession *self,
                          GPid        pid)
{
        GHashTableIter iter;
        gpointer key, value;
        ...
        while (g_hash_table_iter_next (&iter, &key, &value)) {
                ...
                conversation = (ScdmSessionConversation *) value;
                       ...
                       ScdmSession        *self)
{
        ...
        g_debug ("ScdmSession: User not allowed");
...
static gboolean
register_worker (ScdmDBusWorkerManager  *worker_manager_interface,
                 GDBusMethodInvocation *invocation,
                 ScdmSession            *self)
{
        ScdmSessionConversation *conversation;
        ...
        g_debug ("ScdmSession: Authenticating new connection");

        connection = g_dbus_method_invocation_get_connection (invocation);
        connection_node = g_list_find (self->pending_worker_connections, connection);

        if (connection_node == NULL) {
                g_debug ("ScdmSession: Ignoring connection that we aren't tracking");
                return FALSE;
        }
        ...
        if (conversation == NULL) {
                g_warning ("ScdmSession: New worker connection is from unknown source");
        ...
        conversation->worker_proxy = scdm_dbus_worker_proxy_new_sync (connection,
                                                                     G_DBUS_PROXY_FLAGS_NONE,
                                                                     NULL,
                                                                     SCDM_WORKER_DBUS_PATH,
                                                                     NULL, NULL);
        ...
        g_debug ("ScdmSession: worker connection is %p", connection);

        g_debug ("ScdmSession: Emitting conversation-started signal");
        g_signal_emit (self, signals[CONVERSATION_STARTED], 0, conversation->service_name);

        if (self->user_verifier_interface != NULL) {
                scdm_dbus_user_verifier_emit_conversation_started (self->user_verifier_interface,
                                                                  conversation->service_name);
        }

        if (conversation->starting_invocation != NULL) {
                if (conversation->starting_username != NULL) {
                        scdm_session_setup_for_user (self, conversation->service_name, conversation->starting_username);

                        g_clear_pointer (&conversation->starting_username,
                                         (GDestroyNotify)
                                         g_free);
                } else {
                        scdm_session_setup (self, conversation->service_name);
                }
        }

        g_debug ("ScdmSession: Conversation started");

        return TRUE;
}

static void
export_worker_manager_interface (ScdmSession      *self,
                                 GDBusConnection *connection)
{
        ScdmDBusWorkerManager *worker_manager_interface;

        worker_manager_interface = SCDM_DBUS_WORKER_MANAGER (scdm_dbus_worker_manager_skeleton_new ());
        ...
        g_signal_connect (worker_manager_interface,
                          "handle-info-query",
                          G_CALLBACK (scdm_session_handle_info_query),
                          self);
        g_signal_connect (worker_manager_interface,
                          "handle-secret-info-query",
                          G_CALLBACK (scdm_session_handle_secret_info_query),
                          self);
        g_signal_connect (worker_manager_interface,
                          "handle-info",
                          G_CALLBACK (scdm_session_handle_info),
                          self);
        g_signal_connect (worker_manager_interface,
                          "handle-problem",
                          G_CALLBACK (scdm_session_handle_problem),
                          self);
        g_signal_connect (worker_manager_interface,
                          "handle-choice-list-query",
                          G_CALLBACK (scdm_session_handle_choice_list_query),
                          self);
...
static void
unexport_worker_manager_interface (ScdmSession           *self,
                                   ScdmDBusWorkerManager *worker_manager_interface)
{
                               ...
                               ScdmSession       *self)
{
        ...
        g_debug ("ScdmSession: Handling new connection from worker");
...
static ScdmSessionConversation *
begin_verification_conversation (ScdmSession            *self,
                                 GDBusMethodInvocation *invocation,
                                 const char            *service_name)
{
        ScdmSessionConversation *conversation = NULL;
        gboolean conversation_started;

        conversation_started = scdm_session_start_conversation (self, service_name);
...
static gboolean
scdm_session_handle_client_select_choice (ScdmDBusUserVerifierChoiceList    *choice_list_interface,
                                         GDBusMethodInvocation            *invocation,
                                         const char                       *service_name,
                                         const char                       *answer,
                                         ScdmSession                       *self)
{
        g_debug ("ScdmSession: user selected choice '%s'", answer);
        scdm_dbus_user_verifier_choice_list_complete_select_choice (choice_list_interface, invocation);
        scdm_session_answer_query (self, service_name, answer);
        return TRUE;
}

static void
export_user_verifier_choice_list_interface (ScdmSession      *self,
                                            GDBusConnection *connection)
{
        ScdmDBusUserVerifierChoiceList   *interface;

        interface = SCDM_DBUS_USER_VERIFIER_CHOICE_LIST (scdm_dbus_user_verifier_choice_list_skeleton_new ());

        g_signal_connect (interface,
                          "handle-select-choice",
                          G_CALLBACK (scdm_session_handle_client_select_choice),
                          self);
...
static gboolean
scdm_session_handle_client_enable_extensions (ScdmDBusUserVerifier    *user_verifier_interface,
                                             GDBusMethodInvocation  *invocation,
                                             const char * const *    extensions,
                                             GDBusConnection        *connection)
{
        ScdmSession *self = g_object_get_data (G_OBJECT (connection), "scdm-session");
        size_t i;
        ...
        for (i = 0; extensions[i] != NULL; i++) {
                ...
                if (strcmp (extensions[i],
                            scdm_dbus_user_verifier_choice_list_interface_info ()->name) == 0)
                        export_user_verifier_choice_list_interface (self, connection);

        }

        scdm_dbus_user_verifier_complete_enable_extensions (user_verifier_interface, invocation);

        return TRUE;
}
static gboolean
scdm_session_handle_client_begin_verification (ScdmDBusUserVerifier    *user_verifier_interface,
                                              GDBusMethodInvocation  *invocation,
                                              const char             *service_name,
                                              ScdmSession             *self)
{
...
static gboolean
scdm_session_handle_client_begin_verification_for_user (ScdmDBusUserVerifier    *user_verifier_interface,
                                                       GDBusMethodInvocation  *invocation,
                                                       const char             *service_name,
                                                       const char             *username,
                                                       ScdmSession             *self)
...
static gboolean
scdm_session_handle_client_answer_query (ScdmDBusUserVerifier    *user_verifier_interface,
                                        GDBusMethodInvocation  *invocation,
                                        const char             *service_name,
                                        const char             *answer,
                                        ScdmSession             *self)
{
        scdm_dbus_user_verifier_complete_answer_query (user_verifier_interface,
                                                      invocation);
        scdm_session_answer_query (self, service_name, answer);
        return TRUE;
}
...
static gboolean
scdm_session_handle_client_cancel (ScdmDBusUserVerifier    *user_verifier_interface,
                                  GDBusMethodInvocation  *invocation,
                                  ScdmSession             *self)
{
        scdm_dbus_user_verifier_complete_cancel (user_verifier_interface,
                                                invocation);
        scdm_session_cancel (self);
        return TRUE;
}

static gboolean
scdm_session_handle_client_select_session (ScdmDBusGreeter         *greeter_interface,
                                          GDBusMethodInvocation  *invocation,
                                          const char             *session,
                                          ScdmSession             *self)
{
        if (scdm_session_is_running (self)) {
                const char *username;

                username = scdm_session_get_username (self);
                g_debug ("ScdmSession: refusing to select session %s since it's already running (for user %s)",
                         session,
                         username);
                g_dbus_method_invocation_return_error (invocation,
                                                       G_DBUS_ERROR,
        ...
        if (self->greeter_interface != NULL) {
                scdm_dbus_greeter_complete_select_session (greeter_interface,
                                                          invocation);
        }
        scdm_session_select_session (self, session);
        return TRUE;
}

static gboolean
scdm_session_handle_client_select_user (ScdmDBusGreeter        *greeter_interface,
                                       GDBusMethodInvocation *invocation,
                                       const char            *username,
                                       ScdmSession            *self)
{
        if (scdm_session_is_running (self)) {
                ...
                session_username = scdm_session_get_username (self);
                g_debug ("ScdmSession: refusing to select user %s, since session (%p) already running (for user %s)",
                          username,
                          self,
                          session_username);
                g_dbus_method_invocation_return_error (invocation,
        ...
        if (self->greeter_interface != NULL) {
                scdm_dbus_greeter_complete_select_user (greeter_interface,
                                                       invocation);
        }
        g_debug ("ScdmSession: client selected user '%s' on session (%p)", username, self);
        scdm_session_select_user (self, username);
        return TRUE;
}

static gboolean
scdm_session_handle_client_start_session_when_ready (ScdmDBusGreeter        *greeter_interface,
                                                    GDBusMethodInvocation *invocation,
                                                    const char            *service_name,
                                                    ...
                                                    ScdmSession            *self)
{
        if (scdm_session_is_running (self)) {
                ...
                username = scdm_session_get_username (self);
                g_debug ("ScdmSession: refusing to start session (%p), since it's already running (for user %s)",
                         self,
                         username);
                g_dbus_method_invocation_return_error (invocation,
                                                       G_DBUS_ERROR,
        ...
        if (self->greeter_interface != NULL) {
                scdm_dbus_greeter_complete_start_session_when_ready (greeter_interface,
                                                                    invocation);
        }
...
static gboolean
scdm_session_handle_get_timed_login_details (ScdmDBusGreeter        *greeter_interface,
                                            GDBusMethodInvocation *invocation,
                                            ScdmSession            *self)
{
        if (scdm_session_is_running (self)) {
                ...
                username = scdm_session_get_username (self);
                g_debug ("ScdmSession: refusing to give timed login details, session (%p) already running (for user %s)",
                         self,
                         username);
                g_dbus_method_invocation_return_error (invocation,
                                                       G_DBUS_ERROR,
        ...
        if (self->greeter_interface != NULL) {
                scdm_dbus_greeter_complete_get_timed_login_details (greeter_interface,
                                                                   invocation,
                ...
                if (self->timed_login_username != NULL) {
                        scdm_dbus_greeter_emit_timed_login_requested (self->greeter_interface,
...
static gboolean
scdm_session_handle_client_begin_auto_login (ScdmDBusGreeter        *greeter_interface,
                                            GDBusMethodInvocation *invocation,
                                            const char            *username,
                                            ScdmSession            *self)
{
        ...
        if (scdm_session_is_running (self)) {
                session_username = scdm_session_get_username (self);
                g_debug ("ScdmSession: refusing auto login operation, session (%p) already running for user %s (%s requested)",
                         self,
                         session_username,
                         username);
                g_dbus_method_invocation_return_error (invocation,
        ...
        if (self->greeter_interface != NULL) {
                scdm_dbus_greeter_complete_begin_auto_login (greeter_interface,
                                                            invocation);
        }

        g_debug ("ScdmSession: client requesting automatic login for user '%s' on session '%s' (%p)",
                 username,
                 scdm_session_get_session_id (self),
                 self);

        scdm_session_setup_for_user (self, "scdm-autologin", username);

        return TRUE;
}

static void
export_user_verifier_interface (ScdmSession      *self,
                                GDBusConnection *connection)
{
        ScdmDBusUserVerifier   *user_verifier_interface;
        user_verifier_interface = SCDM_DBUS_USER_VERIFIER (scdm_dbus_user_verifier_skeleton_new ());

        g_object_set_data (G_OBJECT (connection), "scdm-session", self);

        g_signal_connect (user_verifier_interface,
                          "handle-enable-extensions",
                          G_CALLBACK (scdm_session_handle_client_enable_extensions),
                          connection);
        g_signal_connect (user_verifier_interface,
                          "handle-begin-verification",
                          G_CALLBACK (scdm_session_handle_client_begin_verification),
                          self);
        g_signal_connect (user_verifier_interface,
                          "handle-begin-verification-for-user",
                          G_CALLBACK (scdm_session_handle_client_begin_verification_for_user),
                          self);
        g_signal_connect (user_verifier_interface,
                          "handle-answer-query",
                          G_CALLBACK (scdm_session_handle_client_answer_query),
                          self);
        g_signal_connect (user_verifier_interface,
                          "handle-cancel",
                          G_CALLBACK (scdm_session_handle_client_cancel),
                          self);
...
static void
export_greeter_interface (ScdmSession      *self,
                          GDBusConnection *connection)
{
        ScdmDBusGreeter *greeter_interface;

        greeter_interface = SCDM_DBUS_GREETER (scdm_dbus_greeter_skeleton_new ());

        g_signal_connect (greeter_interface,
                          "handle-begin-auto-login",
                          G_CALLBACK (scdm_session_handle_client_begin_auto_login),
                          self);
        g_signal_connect (greeter_interface,
                          "handle-select-session",
                          G_CALLBACK (scdm_session_handle_client_select_session),
                          self);
        g_signal_connect (greeter_interface,
                          "handle-select-user",
                          G_CALLBACK (scdm_session_handle_client_select_user),
                          self);
        g_signal_connect (greeter_interface,
                          "handle-start-session-when-ready",
                          G_CALLBACK (scdm_session_handle_client_start_session_when_ready),
                          self);
        g_signal_connect (greeter_interface,
                          "handle-get-timed-login-details",
                          G_CALLBACK (scdm_session_handle_get_timed_login_details),
                          self);
...
static gboolean
scdm_session_handle_client_disconnect (ScdmDBusChooser        *chooser_interface,
                                      GDBusMethodInvocation *invocation,
                                      ScdmSession            *self)
{
        scdm_dbus_chooser_complete_disconnect (chooser_interface,
                                              invocation);
...
static void
export_remote_greeter_interface (ScdmSession      *self,
                                 GDBusConnection *connection)
{
        ScdmDBusRemoteGreeter *remote_greeter_interface;

        remote_greeter_interface = SCDM_DBUS_REMOTE_GREETER (scdm_dbus_remote_greeter_skeleton_new ());

        g_signal_connect (remote_greeter_interface,
                          "handle-disconnect",
                          G_CALLBACK (scdm_session_handle_client_disconnect),
                          self);
...
static gboolean
scdm_session_handle_client_select_hostname (ScdmDBusChooser        *chooser_interface,
                                           GDBusMethodInvocation *invocation,
                                           const char            *hostname,
                                           ScdmSession            *self)
{

        scdm_dbus_chooser_complete_select_hostname (chooser_interface,
                                                   invocation);
...
static void
export_chooser_interface (ScdmSession      *self,
                          GDBusConnection *connection)
{
        ScdmDBusChooser *chooser_interface;

        chooser_interface = SCDM_DBUS_CHOOSER (scdm_dbus_chooser_skeleton_new ());

        g_signal_connect (chooser_interface,
                          "handle-select-hostname",
                          G_CALLBACK (scdm_session_handle_client_select_hostname),
                          self);
                              ...
                              ScdmSession      *self)
{
        ...
        g_debug ("ScdmSession: external connection closed");
                                ...
                                ScdmSession       *self)
{
        ...
        g_debug ("ScdmSession: Handling new connection from outside");
...
static void
setup_worker_server (ScdmSession *self)
{
        GDBusAuthObserver *observer;
        GDBusServer *server;
        GError *error = NULL;

        g_debug ("ScdmSession: Creating D-Bus server for worker for session");
        ...
        server = scdm_dbus_setup_private_server (observer, &error);
        ...
        g_debug ("ScdmSession: D-Bus server for workers listening on %s",
...
static void
setup_outside_server (ScdmSession *self)
{
        GDBusAuthObserver *observer;
        GDBusServer *server;
        GError *error = NULL;

         g_debug ("ScdmSession: Creating D-Bus server for greeters and such for session %s (%p)",
                  scdm_session_get_session_id (self),
                  self);
        ...
        g_debug ("ScdmSession: D-Bus server for greeters listening on %s",
        g_dbus_server_get_client_address (self->outside_server));
}

static void
free_conversation (ScdmSessionConversation *conversation)
{
...
static void
load_lang_config_file (ScdmSession *self)
{
...
static void
scdm_session_init (ScdmSession *self)
{
...
static void
worker_started (ScdmSessionWorkerJob    *job,
                ScdmSessionConversation *conversation)
{
        g_debug ("ScdmSession: Worker job started");

}

static void
worker_exited (ScdmSessionWorkerJob    *job,
               int                     code,
               ScdmSessionConversation *conversation)
{
        ScdmSession *self = conversation->session;

        g_debug ("ScdmSession: Worker job exited: %d", code);
        ...
        g_debug ("ScdmSession: Emitting conversation-stopped signal");
        g_signal_emit (self, signals[CONVERSATION_STOPPED], 0, conversation->service_name);
        if (self->user_verifier_interface != NULL) {
                scdm_dbus_user_verifier_emit_conversation_stopped (self->user_verifier_interface,
                                                                  conversation->service_name);
        }
...
static void
worker_died (ScdmSessionWorkerJob    *job,
             int                     signum,
             ScdmSessionConversation *conversation)
{
        ScdmSession *self = conversation->session;

        g_debug ("ScdmSession: Worker job died: %d", signum);
...
static ScdmSessionConversation *
start_conversation (ScdmSession *self,
                    const char *service_name)
{
        ScdmSessionConversation *conversation;
        char                   *job_name;

        conversation = g_new0 (ScdmSessionConversation, 1);
        conversation->session = g_object_ref (self);
        conversation->service_name = g_strdup (service_name);
        conversation->worker_pid = -1;
        conversation->job = scdm_session_worker_job_new ();
        ...
        job_name = g_strdup_printf ("scdm-session-worker [pam/%s]", service_name);
        if (!scdm_session_worker_job_start (conversation->job, job_name)) {
                g_object_unref (conversation->job);
                g_free (conversation->service_name);
                g_free (conversation);
                g_free (job_name);
        ...
        conversation->worker_pid = scdm_session_worker_job_get_pid (conversation->job);

        return conversation;
}

static void
close_conversation (ScdmSessionConversation *conversation)
{
        ScdmSession *self = conversation->session;
...
static void
stop_conversation (ScdmSessionConversation *conversation)
{
        close_conversation (conversation);

        conversation->is_stopping = TRUE;
        scdm_session_worker_job_stop (conversation->job);
}

static void
stop_conversation_now (ScdmSessionConversation *conversation)
{
        close_conversation (conversation);

        scdm_session_worker_job_stop_now (conversation->job);
        g_clear_object (&conversation->job);
}

#ifdef ENABLE_WAYLAND_SUPPORT
void
scdm_session_set_ignore_wayland (ScdmSession *self,
...
gboolean
scdm_session_start_conversation (ScdmSession *self,
                                const char *service_name)
{
        ScdmSessionConversation *conversation;
        ...
        if (conversation != NULL) {
                if (!conversation->is_stopping) {
                        g_warning ("ScdmSession: conversation %s started more than once", service_name);
                        return FALSE;
                }
                g_debug ("ScdmSession: stopping old conversation %s", service_name);
                scdm_session_worker_job_stop_now (conversation->job);
                g_object_unref (conversation->job);
                conversation->job = NULL;
        }

        g_debug ("ScdmSession: starting conversation %s for session (%p)", service_name, self);
...
void
scdm_session_stop_conversation (ScdmSession *self,
                               const char *service_name)
{
        ScdmSessionConversation *conversation;
...
static void
on_initialization_complete_cb (ScdmDBusWorker *proxy,
                               GAsyncResult  *res,
                               gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ...
        if (ret != NULL) {
                ...
                scdm_session_authenticate (self, service_name);
                g_variant_unref (ret);
            ...
            const char *log_file)
{
        ...
        g_debug ("ScdmSession: Beginning initialization");

        conversation = find_conversation_by_name (self, service_name);
        if (conversation != NULL) {
                scdm_dbus_worker_call_initialize (conversation->worker_proxy,
...
void
scdm_session_setup (ScdmSession *self,
                   const char *service_name)
{
...
void
scdm_session_setup_for_user (ScdmSession *self,
                            const char *service_name,
                            const char *username)
{
        ...
        g_debug ("ScdmSession: Set up service %s for username %s on session (%p)",
                 service_name,
                 username,
                 self);
        scdm_session_select_user (self, username);
...
void
scdm_session_setup_for_program (ScdmSession *self,
                               const char *service_name,
                               const char *username,
                               const char *log_file)
{
...
void
scdm_session_authenticate (ScdmSession *self,
                          const char *service_name)
{
        ScdmSessionConversation *conversation;
        ...
        conversation = find_conversation_by_name (self, service_name);
        if (conversation != NULL) {
                scdm_dbus_worker_call_authenticate (conversation->worker_proxy,
                                                   conversation->worker_cancellable,
...
void
scdm_session_authorize (ScdmSession *self,
                       const char *service_name)
{
        ScdmSessionConversation *conversation;
        ...
        conversation = find_conversation_by_name (self, service_name);
        if (conversation != NULL) {
                scdm_dbus_worker_call_authorize (conversation->worker_proxy,
                                                conversation->worker_cancellable,
...
void
scdm_session_accredit (ScdmSession *self,
                      const char *service_name)
{
        ScdmSessionConversation *conversation;
        ...
        conversation = find_conversation_by_name (self, service_name);
        if (conversation != NULL) {
                scdm_dbus_worker_call_establish_credentials (conversation->worker_proxy,
                                                            conversation->worker_cancellable,
...
send_environment_variable (const char             *key,
                           ...
                           ScdmSessionConversation *conversation)
{
        scdm_dbus_worker_call_set_environment_variable (conversation->worker_proxy,
                                                       key, value,
                                                       conversation->worker_cancellable,
                                                       NULL, NULL);
}
...
static void
send_environment (ScdmSession             *self,
                  ScdmSessionConversation *conversation)
{
...
void
scdm_session_send_environment (ScdmSession *self,
                              const char *service_name)
{
        ScdmSessionConversation *conversation;
...
static const char *
get_session_name (ScdmSession *self)
{
...
static char *
get_session_command (ScdmSession *self)
{
        gboolean    res;
        char       *command;
        const char *session_name;
...
static gchar *
get_session_desktop_names (ScdmSession *self)
{
        gchar *filename;
        GKeyFile *keyfile;
        ...
        g_debug ("ScdmSession: getting desktop names for file '%s'", filename);
...
void
scdm_session_set_environment_variable (ScdmSession *self,
                                      const char *key,
                                      const char *value)
{
...
static void
set_up_session_language (ScdmSession *self)
{
        char **environment;
        int i;
        const char *value;
...
static void
set_up_session_environment (ScdmSession *self)
{
        ...
        if (desktop_names != NULL) {
                scdm_session_set_environment_variable (self, "XDG_CURRENT_DESKTOP", desktop_names);
        }
...
static void
send_display_mode (ScdmSession *self,
                   ScdmSessionConversation *conversation)
{
        ScdmSessionDisplayMode mode;

        mode = scdm_session_get_display_mode (self);
        scdm_dbus_worker_call_set_session_display_mode (conversation->worker_proxy,
                                                       scdm_session_display_mode_to_string (mode),
                                                       conversation->worker_cancellable,
                                                       NULL, NULL);
}

static void
send_session_type (ScdmSession *self,
                   ScdmSessionConversation *conversation)
{
...
void
scdm_session_open_session (ScdmSession *self,
                          const char *service_name)
{
        ScdmSessionConversation *conversation;
        ...
        if (conversation != NULL) {
                ...
                scdm_dbus_worker_call_open (conversation->worker_proxy,
                                           conversation->worker_cancellable,
...
static void
stop_all_other_conversations (ScdmSession             *self,
                              ScdmSessionConversation *conversation_to_keep,
                              gboolean                now)
{
        GHashTableIter iter;
        gpointer key, value;
        ...
        if (conversation_to_keep == NULL) {
                g_debug ("ScdmSession: Stopping all conversations");
        } else {
                g_debug ("ScdmSession: Stopping all conversations except for %s",
...
static void
on_start_program_cb (ScdmDBusWorker *worker,
                     GAsyncResult  *res,
                     gpointer       user_data)
{
        ScdmSessionConversation *conversation = user_data;
        ...
        worked = scdm_dbus_worker_call_start_program_finish (worker,
                                                            &pid,
                                                            res,
                                                            &error);
        ...
        if (worked) {
                ...
                g_debug ("ScdmSession: Emitting 'session-started' signal with pid '%d'", pid);
...
void
scdm_session_start_session (ScdmSession *self,
                           const char *service_name)
{
        ScdmSessionConversation *conversation;
        ScdmSessionDisplayMode   display_mode;
        ...
        if (conversation == NULL) {
                g_warning ("ScdmSession: Tried to start session of "
        ...
        register_session = !scdm_session_session_registers (self);
        ...
        if (self->selected_program == NULL) {
                ...
                run_xsession_script = !scdm_session_bypasses_xsession (self);

                if (self->display_is_local) {
                        gboolean disallow_tcp = TRUE;
                        scdm_settings_direct_get_boolean (SCDM_KEY_DISALLOW_TCP, &disallow_tcp);
                ...
                if (run_launcher) {
                        if (is_x11) {
                                program = g_strdup_printf (LIBEXECDIR "/scdm-x-session %s%s %s\"%s\"",
                                                           register_session ? "--register-session " : "",
                                                           run_xsession_script? "--run-script " : "",
                                                           allow_remote_connections? "--allow-remote-connections " : "",
                                                           command);
                        } else {
                                program = g_strdup_printf (LIBEXECDIR "/scdm-wayland-session %s\"%s\"",
                                                           register_session ? "--register-session " : "",
                                                           command);
                        }
                ...
                if (run_launcher) {
                        if (is_x11) {
                                program = g_strdup_printf (LIBEXECDIR "/scdm-x-session %s\"dbus-run-session -- %s\"",
                                                           register_session ? "--register-session " : "",
                                                           self->selected_program);
                        } else {
                                program = g_strdup_printf (LIBEXECDIR "/scdm-wayland-session %s\"dbus-run-session -- %s\"",
                                                           register_session ? "--register-session " : "",
                                                           self->selected_program);
                        }
                } else {
        ...
        scdm_dbus_worker_call_start_program (conversation->worker_proxy,
                                            program,
                                            conversation->worker_cancellable,
...
static void
stop_all_conversations (ScdmSession *self)
{
        stop_all_other_conversations (self, NULL, TRUE);
}

static void
do_reset (ScdmSession *self)
{
...
void
scdm_session_answer_query (ScdmSession *self,
                          const char *service_name,
                          const char *text)
{
        ScdmSessionConversation *conversation;
...
void
scdm_session_reset (ScdmSession *self)
{
        if (self->user_verifier_interface != NULL) {
                scdm_dbus_user_verifier_emit_reset (self->user_verifier_interface);
        }
...
void
scdm_session_set_timed_login_details (ScdmSession *self,
                                     const char *username,
                                     int         delay)
{
        g_debug ("ScdmSession: timed login details %s %d", username, delay);
        self->timed_login_username = g_strdup (username);
        self->timed_login_delay = delay;
}

gboolean
scdm_session_is_running (ScdmSession *self)
{
...
uid_t
scdm_session_get_allowed_user (ScdmSession *self)
{
        return self->allowed_user;
}

void
scdm_session_start_reauthentication (ScdmSession *session,
                                    GPid        pid_of_caller,
                                    uid_t       uid_of_caller)
{
        ScdmSessionConversation *conversation = session->session_conversation;

        g_return_if_fail (conversation != NULL);

        g_debug ("ScdmSession: starting reauthentication for session %s for client with pid %d",
                 conversation->session_id,
                 (int) uid_of_caller);

        conversation->reauth_pid_of_caller = pid_of_caller;

        scdm_dbus_worker_call_start_reauthentication (conversation->worker_proxy,
                                                     (int) pid_of_caller,
                                                     (int) uid_of_caller,
                                                     conversation->worker_cancellable,
...
const char *
scdm_session_get_session_id (ScdmSession *self)
{
        ScdmSessionConversation *conversation;
...
const char *
scdm_session_get_conversation_session_id (ScdmSession *self,
                                         const char *service_name)
{
        ScdmSessionConversation *conversation;
...
static char *
get_session_filename (ScdmSession *self)
{
...
#ifdef ENABLE_WAYLAND_SUPPORT
static gboolean
scdm_session_is_wayland_session (ScdmSession *self)
{
        GKeyFile   *key_file;
        ...
        g_debug ("ScdmSession: checking if file '%s' is wayland session: %s", filename, is_wayland_session? "yes" : "no");
...
static void
update_session_type (ScdmSession *self)
{
#ifdef ENABLE_WAYLAND_SUPPORT
        gboolean is_wayland_session;

        is_wayland_session = scdm_session_is_wayland_session (self);
...
gboolean
scdm_session_session_registers (ScdmSession *self)
{
        ...
        if (!session_registers &&
            ...
            !g_error_matches (error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND)) {
                g_warning ("ScdmSession: Couldn't read session file '%s'", filename);
                return FALSE;
        }

        g_debug ("ScdmSession: '%s' %s self", filename,
...
gboolean
scdm_session_bypasses_xsession (ScdmSession *self)
{
        GError     *error;
        GKeyFile   *key_file;
        gboolean    res;
...
out:
        if (bypasses_xsession) {
                g_debug ("ScdmSession: Session %s bypasses Xsession wrapper script", filename);
        }
        g_free (filename);
...
scdm_session_get_display_mode (ScdmSession *self)
{
        g_debug ("ScdmSession: type %s, program? %s, seat %s",
                 self->session_type,
...
void
scdm_session_select_program (ScdmSession *self,
                            const char *text)
{
...
void
scdm_session_select_session (ScdmSession *self,
                            const char *text)
{
        GHashTableIter iter;
        gpointer key, value;

        g_debug ("ScdmSession: selecting session '%s'", text);
        ...
        while (g_hash_table_iter_next (&iter, &key, &value)) {
                ...
                scdm_dbus_worker_call_set_session_name (conversation->worker_proxy,
                                                       get_session_name (self),
                                                       conversation->worker_cancellable,
                                                       NULL, NULL);
        }
...
static void
set_display_name (ScdmSession *self,
                  const char *name)
{
...
static void
set_display_hostname (ScdmSession *self,
                      const char *name)
{
...
static void
set_display_device (ScdmSession *self,
                    const char *name)
{
        g_debug ("ScdmSession: Setting display device: %s", name);
        g_free (self->display_device);
        self->display_device = g_strdup (name);
}

static void
set_display_seat_id (ScdmSession *self,
                     const char *name)
{
...
static void
set_user_x11_authority_file (ScdmSession *self,
                             const char *name)
{
        g_free (self->user_x11_authority_file);
...
static void
set_display_x11_authority_file (ScdmSession *self,
                                const char *name)
{
...
static void
set_display_is_local (ScdmSession *self,
                      gboolean    is_local)
{
...
static void
set_display_is_initial (ScdmSession *self,
                        gboolean    is_initial)
{
...
static void
set_verification_mode (ScdmSession                 *self,
                       ScdmSessionVerificationMode  verification_mode)
{
...
static void
set_allowed_user (ScdmSession *self,
                  uid_t       allowed_user)
{
...
static void
set_conversation_environment (ScdmSession  *self,
                              char       **environment)
{
...
static void
set_session_type (ScdmSession *self,
                  const char *session_type)
{

        if (g_strcmp0 (self->session_type, session_type) != 0) {
                g_debug ("ScdmSession: setting session to type '%s'", session_type? session_type : "");
...
static void
scdm_session_set_property (GObject      *object,
                          guint         prop_id,
                          const GValue *value,
                          GParamSpec   *pspec)
{
        ...
        case PROP_IGNORE_WAYLAND:
                scdm_session_set_ignore_wayland (self, g_value_get_boolean (value));
                break;
#endif
        default:
...
static void
scdm_session_get_property (GObject    *object,
                          guint       prop_id,
                          GValue     *value,
                          GParamSpec *pspec)
{
...
static void
scdm_session_dispose (GObject *object)
{
        ScdmSession *self;
        ...
        G_OBJECT_CLASS (scdm_session_parent_class)->dispose (object);
}

static void
scdm_session_finalize (GObject *object)
{
        ScdmSession   *self;
        ...
        parent_class = G_OBJECT_CLASS (scdm_session_parent_class);
...
static GObject *
scdm_session_constructor (GType                  type,
...
static void
scdm_session_class_init (ScdmSessionClass *session_class)
{
        GObjectClass *object_class;

        object_class = G_OBJECT_CLASS (session_class);

        object_class->get_property = scdm_session_get_property;
        object_class->set_property = scdm_session_set_property;
        object_class->constructor = scdm_session_constructor;
        object_class->dispose = scdm_session_dispose;
        object_class->finalize = scdm_session_finalize;
...
ScdmSession *
scdm_session_new (ScdmSessionVerificationMode  verification_mode,
                 uid_t                       allowed_user,
                 ...
                 const char * const         *environment)
{
        ...
        self = g_object_new (SCDM_TYPE_SESSION,
...
scdm_session_display_mode_to_string (ScdmSessionDisplayMode mode)
{
        ...
        g_warning ("Unknown ScdmSessionDisplayMode %d", mode);
        return "";
}

GPid
scdm_session_get_pid (ScdmSession *session)
{
